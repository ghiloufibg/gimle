...
/**
 * One accepted worker's half of the control channel from the agent's side (design §4.2). The agent
 * listens; each spawned worker connects out and gets its own {@link WorkerConnection}. IO shape
 * mirrors {@code gimle-worker}'s {@code ControlChannelClient} exactly (same newline-framed,
 * space-separated {@link ControlMessageCodec} wire format) -- deliberately not shared as a common
 * abstraction, since the two sides differ in only one respect (who initiates the connection) and
 * factoring that out would cost more than the few lines it saves.
 */
