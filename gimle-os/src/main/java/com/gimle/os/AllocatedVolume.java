 ...
 * VolumeManager#listAllocated()}: its owning coordinate (tenant included, since two tenants may
 * hold an identically-named {@code statefulSetName} — see {@link VolumeHandle}'s own javadoc), the
 * volume's name within that instance, its host path, and the bytes its files currently occupy (a
 * walk-and-sum at listing time -- an observation, not an enforced limit, matching the advisory
 * posture of {@code VolumeRequest#sizeBytes} itself). "Allocated" here means "present on disk": a
 * retained volume whose instance was permanently removed still lists, which is exactly what makes
 * an operator able to find and reclaim it.
 */
