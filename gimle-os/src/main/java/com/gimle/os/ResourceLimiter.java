 ...
 * identically on every OS. This interface exists because the spec's own architecture names multiple
 * isolation strategies as a load-bearing, defining feature — not because a second implementation is
 * needed today — so callers are written against it and never branch on platform themselves; a
 * future kernel-level implementation drops in without touching a caller.
 */
