...
- **Shipped scope**: request-rate/error-rate flow real through the heartbeat and a `/metrics` rollup endpoint, per the "Partial" fit above. Percentile latency, once deferred here, has since landed as well and is wired end to end: `TaggedRequestMetrics` publishes p50/p95/p99 for `ApiServerMetrics`, `AndvariMetrics`, `StoreMetrics`, `FafnirMetrics`, and both of `WorkerMetrics`' triples (module requests and fabric client calls); `MeterSnapshotCodec` reads them off each `Timer`'s own `HistogramSnapshot` and ships them to Muninn as a `percentiles` field; and the console's `MetricsHistoryPanel` plots p50/p95/p99 alongside the primary series. Two honest limits remain, neither a deferral of the above: `AgentMetrics` deliberately opts out (`publishPercentiles = false` — agent tick latency is not a served-request signal), and percentiles reach the console only through the Muninn-backed `/metrics-history/*` history screen, not through the `/metrics` rollup endpoint, which stays a counter/rate summary.
