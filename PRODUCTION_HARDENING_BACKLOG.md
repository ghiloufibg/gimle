...
Gap analysis of the codebase at `master` (`d501cb2`), benchmarked against how Kubernetes,
Kafka (KRaft), Elasticsearch, and Spark actually solve the same problems. Gimlé is an
educational JPMS/`ModuleLayer` teaching project, not a product aimed at real users — the
goal here is not "compete with these systems," it's "where does the implementation stop
matching the production pattern it's teaching, and is that gap a deliberate, documented
simplification or an unintentional one worth closing."
...
a specific named mechanism in a specific real system.
...
- **Real-world comparison**: etcd's raft WAL calls `fileutil.Fsync` before an entry counts as persisted; Kafka's metadata log fsyncs per its flush policy before advancing the high-watermark used for acks.
...
- **Real-world comparison**: Kubernetes' `nodeSelector`/`nodeAffinity` actually filter the candidate set in `kube-scheduler`.
...
- **Real-world comparison**: Kubernetes' `CrashLoopBackOff` never gives up — it caps backoff at 5 minutes and keeps retrying, so there's always a next action.
...
- **Real-world comparison**: Kubernetes' `preStop` + endpoint deregistration + grace period genuinely waits for real in-flight connections; Kafka broker shutdown drains real in-flight requests.
...
- **Real-world comparison**: kubelet/cAdvisor poll real cgroup counters feeding HPA and OOM decisions; Kafka/ES record request rate/latency/errors directly in the request path, not as disconnected instrumentation.
...
- **Real-world comparison**: Kubernetes stores `CrashLoopBackOff`-relevant state in the Pod's own etcd-backed status precisely so controller-manager restarts don't reset it.
...
- **Real-world comparison**: Kubernetes removed `--insecure-port` specifically because default-open API access was a recurring CVE source; Elasticsearch ships security features on by default since 8.0.
...
- **Real-world comparison**: Kubernetes RBAC treats `configmaps` and `secrets` as distinct resource types so a role can grant one without the other.
...
- **Real-world comparison**: Envoy cluster config always sets `connect_timeout` + per-route `timeout`; a timed-out upstream counts as an error for outlier detection just like a connection failure.
...
- **Real-world comparison**: Kubernetes packs many pods onto one kubelet/containerd node; Gimlé's own pitch is the JVM-analogue of that for Tier 1 — many modules in *one* worker JVM.
...
- **Real-world comparison**: Kubernetes' Events API / `kubectl describe pod` surfacing `BackOff`/`Killing`/`Started` with counts.
...
- **Real-world comparison**: Kafka/ES dashboards surface per-topic/per-index request rate, p99 latency, error rate as first-class panels.
...
- **Real-world comparison**: Kafka carries `api_key`+`api_version` on every request and negotiates via `ApiVersionsRequest`, enabling rolling upgrades.
...
- **Real-world comparison**: Kubernetes runs predicates + a weighted sum of multiple priority functions + taints/tolerations + preemption.
...
| P2-6 | Circuit breaker conflates application errors with transport/availability failures — a legitimate validation exception scores identically to a network failure (`FabricServiceRegistry.java:282-298`) | gimle-fabric | Medium | Medium | Yes |
| P2-7 | No cluster-wide ejection safety valve ("panic threshold") — a correlated failure can open every endpoint's breaker simultaneously with `lookup` just returning empty (`CircuitBreaker.java`, `FabricServiceRegistry.java:224-236`) | gimle-fabric | Medium | Small | Yes |
| P2-8 | SWIM gossip has no periodic anti-entropy full-state sync — bounded 64-entry piggyback history can let a slow/partitioned node permanently miss an update (`GossipMember.java:594-626`) | gimle-fabric | Medium | Medium | Yes |
| P2-9 | SWIM probe scheduling is pure-random with a fixed suspicion timeout — no per-cycle coverage guarantee, no Lifeguard-style adaptive suspicion (`GossipMember.java:250-259`, `GossipConfig.java`) | gimle-fabric | Medium | Medium | Yes |
