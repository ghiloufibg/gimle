...
| GIMLE-044 | Module registry (install bookkeeping, idempotent re-install, content-mismatch reject… | Active | Not Covered | — |
| GIMLE-045 | Module lifecycle state machine (INSTALLED→RESOLVED→STARTING→ACTIVE→STOPPING→UNINSTAL… | Active | Not Covered | — |
...
| GIMLE-053 | Module context API (in-flight tracking, service lookup, config, data dir, control-pl… | Active | Not Covered | — |
...
| GIMLE-089 | Module-tier self-healing — restart on repeated liveness failure with backoff and bud… | Active | Covered | `self-healing.feature` — "A module that never passes liveness is escalated to FAILED for good" |
...
| GIMLE-107 | Portable JVM-flags resource limiting (Tier 1/2), cgroup enforcement deliberately def… | Active | Not Covered | — |
...
| GIMLE-131 | Whitelisted control-plane read relay (worker→agent→control plane) with independent r… | Active | Not Covered | — |
...
| GIMLE-377 | Instance lifecycle event timeline | Active | Not Covered | — |
...
| GIMLE-387 | Certificate lifecycle management (bootstrap token, CSR request/status/approve, renew… | Active | Not Covered | — |
| GIMLE-388 | Dual table/JSON output formatting | Active | Not Covered | — |
| GIMLE-389 | kubectl-shaped global flag parsing, manifest-kind apply dispatch, and mTLS/leader-aw… | Active | Not Covered | — |
...
| GIMLE-397 | Per-machine platform binary rolling upgrade with quorum-safe store restart (`hilmir… | Active | Not Covered | — |
...
| GIMLE-419 | `mvn gimle:bootstrap` — full local-dev cluster orchestration in one foreground comma… | Active | Not Covered | — |
| GIMLE-420 | Process-launcher Maven goals for individual platform processes (`controlplane`/`stor… | Active | Not Covered | — |
...
| GIMLE-426 | `mvn gimle:flaky-tests` — run known-flaky-tagged tests in isolated standalone reacto… | Active | Not Covered | — |
...
| GIMLE-434 | Internal — surefire report discovery and totals aggregation, including flaky-testcas… | Active | Not Covered | — |
...
| GIMLE-504 | `greeter-provider` — real fabric service export with lifecycle hooks and health prob… | Active | Covered | `deployment-lifecycle.feature` — "A consumer completes a real fabric call to a provider" |
...
| GIMLE-506 | `greeter-load-generator` — HTTP bridge for external load tools driving real fabric t… | Active | Covered | `autoscale.feature` — "Request-rate load scales the provider up" |
...
| GIMLE-521 | Autoscaling under real request-rate, error-rate, queue-depth, and weighted-blended l… | Active | Not Covered | — |
...
| GIMLE-525 | Observability data survives agent death (Muninn fallback) and control-plane metrics… | Active | Not Covered | — |
...
|---|---|---|---|---|
| GIMLE-314 | gimle-andvari | Andvari's own console session story (`/auth/*`, bundled SPA) | API Server | Yes |
| GIMLE-318 | gimle-andvari | Process status endpoint (no RBAC gate) | API Server | Yes |
| GIMLE-290 | gimle-fafnir | Console session login (Fafnir's own operator dashboard) | API Server | Yes |
| GIMLE-272 | gimle-controlplane | Bundled web console static serving | API Server / Internal-Infra | Yes |
| GIMLE-292 | gimle-fafnir | Bundled web console static serving (Fafnir) | API Server / Internal-Infra | Yes |
| GIMLE-293 | gimle-fafnir | Process status endpoint with key-ring fingerprint | API Server / Internal-Infra | Yes |
| GIMLE-275 | gimle-controlplane | Per-deployment and per-instance metrics rollup | API Server / Observability | Yes |
| GIMLE-269 | gimle-controlplane | Node registration, heartbeat, and assignment-fetch API | API Server / Orchestration | Yes |
| GIMLE-247 | gimle-controlplane | Organization-specific policy-as-data admission (`policy.maxReplicasPer | Admission / Config | Yes |
| GIMLE-245 | gimle-controlplane | Admission chain extension point | Admission / Internal-Infra | Yes |
| GIMLE-297 | gimle-andvari | Immutable, content-addressed artifact store | Artifact Registry | Yes |
| GIMLE-299 | gimle-andvari | Size-limited streaming upload rejection | Artifact Registry | Yes |
| GIMLE-302 | gimle-andvari | Version retention sweeping (count and age based) | Artifact Registry | Yes |
| GIMLE-308 | gimle-andvari | Generated `maven-metadata.xml` (never stored, always fresh) | Artifact Registry | Yes |
| GIMLE-306 | gimle-andvari | Maven-2-shaped `/repository/**` interop surface | Artifact Registry / API Server | Yes |
| GIMLE-298 | gimle-andvari | Streamed, digest-verified push with atomic commit | Artifact Registry / Internal-Infra | Yes |
| GIMLE-300 | gimle-andvari | On-disk corruption detection and quarantine | Artifact Registry / Internal-Infra | Yes |
| GIMLE-301 | gimle-andvari | Periodic full-store integrity scrub | Artifact Registry / Internal-Infra | Yes |
| GIMLE-304 | gimle-andvari | Peer-sync conflict detection (irreconcilable divergence) | Artifact Registry / Internal-Infra | Yes |
| GIMLE-307 | gimle-andvari | Server-computed checksum sidecars (never trusting client uploads) | Artifact Registry / Internal-Infra | Yes |
| GIMLE-309 | gimle-andvari | Maven GAV coordinate translation | Artifact Registry / Internal-Infra | Yes |
| GIMLE-265 | gimle-controlplane | `/artifacts/*` streaming proxy to Andvari | Artifact Registry / Internal-Infra | Yes |
| GIMLE-266 | gimle-controlplane | Andvari-client multi-endpoint failover with rotation | Artifact Registry / Internal-Infra | Yes |
| GIMLE-310 | gimle-andvari | Defense-in-depth authorization (independent re-check, `ResourceKind.AR | Authorization | Yes |
| GIMLE-311 | gimle-andvari | Module-scoped permission grants | Authorization | Yes |
| GIMLE-312 | gimle-andvari | Node pull-only artifact access, scoped to active assignments | Authorization | Yes |
| GIMLE-249 | gimle-controlplane | PUT-time re-tenanting double-authorization | Authorization | Yes |
| GIMLE-250 | gimle-controlplane | RBAC-gated resource CRUD across every workload kind | Authorization | Yes |
| GIMLE-252 | gimle-controlplane | `gimle-system` reserved-tenant operator-only guard | Authorization | Yes |
| GIMLE-253 | gimle-controlplane | Node-scoped self-service authorization (`gimle:nodes` group) | Authorization | Yes |
| GIMLE-254 | gimle-controlplane | Node-tenant-scoped `/endpoints/*` read access | Authorization | Yes |
| GIMLE-285 | gimle-fafnir | Fafnir's own independent RBAC re-check (defense-in-depth) | Authorization | Yes |
| GIMLE-286 | gimle-fafnir | Node-tenant-scoped secret reads (`gimle:nodes`) | Authorization | Yes |
| GIMLE-291 | gimle-fafnir | Plaintext-mode anonymous session carve-out | Authorization | Yes |
| GIMLE-256 | gimle-controlplane | Console session login/logout/session cookie flow | Authorization / API Server | Yes |
| GIMLE-251 | gimle-controlplane | WRITE/DELETE decisions durably audited (opt-in READ auditing) | Authorization / Internal-Infra | Yes |
| GIMLE-257 | gimle-controlplane | Login throttling (address + username keyed) | Authorization / Internal-Infra | Yes |
| GIMLE-287 | gimle-fafnir | Authorization-failure throttling and dual audit logging | Authorization / Internal-Infra | Yes |
| GIMLE-409 | gimle-hilmir | Doctor static deployability diagnostics (`hilmir doctor`) | Build Tooling | Yes |
| GIMLE-410 | gimle-hilmir | Doctor cluster-aware checks (`--server`, `--tenant`) | Build Tooling | None |
| GIMLE-411 | gimle-hilmir | Manifest scaffolding (`hilmir init`) | Build Tooling | Yes |
| GIMLE-418 | gimle-maven-plugin | `mvn gimle:agent` — spawn a real node agent (plus its worker command t | Build Tooling | None |
| GIMLE-419 | gimle-maven-plugin | `mvn gimle:bootstrap` — full local-dev cluster orchestration in one fo | Build Tooling | None |
| GIMLE-420 | gimle-maven-plugin | Process-launcher Maven goals for individual platform processes (`contr | Build Tooling | None |
| GIMLE-421 | gimle-maven-plugin | `mvn gimle:deploy` — apply a deployment manifest via a real CLI subpro | Build Tooling | None |
| GIMLE-422 | gimle-maven-plugin | `mvn gimle:doctor` — run hilmir doctor against the invoking project's  | Build Tooling | Yes |
| GIMLE-423 | gimle-maven-plugin | `mvn gimle:init` — scaffold manifests for the invoking project's own b | Build Tooling | Yes |
| GIMLE-424 | gimle-maven-plugin | `mvn gimle:publish` — push a built module jar to the artifact registry | Build Tooling | None |
| GIMLE-425 | gimle-maven-plugin | `mvn gimle:docs` — full documentation site build pipeline | Build Tooling | None |
| GIMLE-426 | gimle-maven-plugin | `mvn gimle:flaky-tests` — run known-flaky-tagged tests in isolated sta | Build Tooling | Yes |
| GIMLE-427 | gimle-maven-plugin | `mvn gimle:saga` — ensure a Saga test-report server is running | Build Tooling | Yes |
| GIMLE-428 | gimle-maven-plugin | `mvn gimle:verify` — full build run under Saga tracking | Build Tooling | Yes |
| GIMLE-429 | gimle-maven-plugin | `mvn gimle:saga-import` — standalone sweep-and-import of existing sure | Build Tooling | None |
| GIMLE-430 | gimle-maven-plugin | `mvn gimle:saga-stop` — best-effort local Saga server shutdown | Build Tooling | None |
| GIMLE-371 | gimle-cli | Deployment resource management (get/apply/delete) | CLI | Yes |
| GIMLE-372 | gimle-cli | Job resource management (get/apply/delete) | CLI | Yes |
| GIMLE-373 | gimle-cli | CronJob management incl. manual trigger | CLI | Yes |
| GIMLE-374 | gimle-cli | DaemonSet resource management | CLI | Yes |
| GIMLE-375 | gimle-cli | StatefulSet resource management | CLI | Yes |
| GIMLE-376 | gimle-cli | Node inventory and cordon/uncordon | CLI | Yes |
| GIMLE-377 | gimle-cli | Instance lifecycle event timeline | CLI | None |
| GIMLE-378 | gimle-cli | Tenant management and quota configuration | CLI | Yes |
| GIMLE-379 | gimle-cli | Tenant plain configuration key/value store | CLI | Yes |
| GIMLE-382 | gimle-cli | Log viewing and live tailing | CLI | None |
| GIMLE-381 | gimle-cli | Artifact registry client (push/list/get/delete) | CLI / Build Tooling | None |
| GIMLE-388 | gimle-cli | Dual table/JSON output formatting | CLI / Internal-Infra | Yes |
| GIMLE-380 | gimle-cli | Versioned secrets management (Fafnir proxy) | CLI / Security | Yes |
| GIMLE-383 | gimle-cli | Audit trail query | CLI / Security | Yes |
| GIMLE-384 | gimle-cli | RBAC role management | CLI / Security | Yes |
| GIMLE-385 | gimle-cli | RBAC role binding management | CLI / Security | Yes |
| GIMLE-386 | gimle-cli | Operator account management | CLI / Security | Yes |
| GIMLE-387 | gimle-cli | Certificate lifecycle management (bootstrap token, CSR request/status/ | CLI / Security | None |
| GIMLE-107 | gimle-agent | Portable JVM-flags resource limiting (Tier 1/2), cgroup enforcement de | Cgroup Management | Yes |
| GIMLE-108 | gimle-agent | Tier 3 isolation rejection | Cgroup Management / Config | None |
| GIMLE-186 | gimle-fabric | Per-Endpoint Circuit Breaker | Circuit Breaking | Yes |
| GIMLE-187 | gimle-fabric | Circuit Breaker Exponential Cooldown Backoff | Circuit Breaking | Yes |
| GIMLE-188 | gimle-fabric | Panic-Mode Ejection Floor | Circuit Breaking | Yes |
| GIMLE-189 | gimle-fabric | Application-Exception vs Transport-Failure Breaker Scoring | Circuit Breaking | Yes |
| GIMLE-554 | gimle-holmgang | Utgard multi-container distributed boot ordering | Cluster Validation | Yes |
| GIMLE-555 | gimle-holmgang | Utgard real machine loss (hard container kill) and rejoin | Cluster Validation | Yes |
| GIMLE-556 | gimle-holmgang | Utgard network partition (vs hard kill) with reconvergence | Cluster Validation | Yes |
| GIMLE-557 | gimle-holmgang | Utgard real-hostname mTLS bootstrap across containers | Cluster Validation | Yes |
| GIMLE-509 | gimle-smoke-tests | Base cluster topology deploy across store cluster and multiple CP repl | Cluster Validation | Yes |
| GIMLE-510 | gimle-smoke-tests | Raft store resilience (member loss, leader failover, live membership c | Cluster Validation | Yes |
| GIMLE-511 | gimle-smoke-tests | Tiered self-healing (worker respawn, liveness-exhaustion escalation to | Cluster Validation | Yes |
| GIMLE-512 | gimle-smoke-tests | Classloader leak detection wired into a real worker | Cluster Validation | Yes |
| GIMLE-513 | gimle-smoke-tests | Repeated redeploy stability without false-positive leaks | Cluster Validation | Yes |
| GIMLE-514 | gimle-smoke-tests | Tier 1 worker density packing and its cap | Cluster Validation | Yes |
| GIMLE-515 | gimle-smoke-tests | Node cordoning blocks new placement without evicting running instances | Cluster Validation | Yes |
| GIMLE-516 | gimle-smoke-tests | DaemonSet per-node fan-out and dead-node assignment cleanup | Cluster Validation | Yes |
| GIMLE-517 | gimle-smoke-tests | Job and CronJob real-cluster lifecycle | Cluster Validation | Yes |
| GIMLE-518 | gimle-smoke-tests | StatefulSet sticky placement and volume persistence across worker rest | Cluster Validation | Yes |
| GIMLE-519 | gimle-smoke-tests | Rolling update preserves serving capacity and reaches new version | Cluster Validation | Yes |
| GIMLE-520 | gimle-smoke-tests | Surge worker promotion carries out via in-place retarget, not respawn | Cluster Validation | Yes |
| GIMLE-522 | gimle-smoke-tests | Multi-tenant quota enforcement (flag-not-evict, and admission rejectio | Cluster Validation | Yes |
| GIMLE-523 | gimle-smoke-tests | Circuit breaker excludes a consistently-failing replica | Cluster Validation | Yes |
| GIMLE-524 | gimle-smoke-tests | Gossip/SWIM failure detection across real separate agent processes | Cluster Validation | Yes |
| GIMLE-525 | gimle-smoke-tests | Observability data survives agent death (Muninn fallback) and control- | Cluster Validation | Yes |
| GIMLE-526 | gimle-smoke-tests | Worker-tier metrics/trace relay to Muninn via the agent | Cluster Validation | Yes |
| GIMLE-527 | gimle-smoke-tests | Artifact registry (Andvari) resolution path end to end | Cluster Validation | Yes |
| GIMLE-528 | gimle-smoke-tests | External HTTP request reaches a fabric service through the gateway | Cluster Validation | Yes |
| GIMLE-117 | gimle-agent | Persistent volume allocation for StatefulSet-shaped instances | Config | None |
| GIMLE-119 | gimle-agent | Vessel port allocation (dynamic/fixed) and env resolution (literal/por | Config | None |
| GIMLE-120 | gimle-agent | Vessel config-file rendering to disk | Config | None |
| GIMLE-134 | gimle-agent | Node placement-label registration | Config | None |
| GIMLE-015 | gimle-core | Cluster-wide transport protocol switch (plaintext/TLS) | Config | Yes |
| GIMLE-363 | gimle-gateway | Route-table config DSL parsing | Config | Yes |
| GIMLE-364 | gimle-gateway | Duplicate route-path rejection at config-parse time | Config | Yes |
| GIMLE-171 | gimle-mimir | Five-Field Cron Schedule Evaluator | Config | Yes |
| GIMLE-172 | gimle-mimir | Deployment Manifest Parsing (incl. Autoscale & Disruption Budget) | Config | Yes |
| GIMLE-173 | gimle-mimir | DaemonSet Manifest Parsing (Anti-Affinity/Surge Rejection) | Config | Yes |
| GIMLE-174 | gimle-mimir | Job / CronJob Manifest Parsing | Config | Yes |
| GIMLE-175 | gimle-mimir | StatefulSet Manifest Parsing | Config | Yes |
| GIMLE-176 | gimle-mimir | Kind-Dispatching Manifest Parser | Config | Yes |
| GIMLE-332 | gimle-muninn | Plaintext-default transport with loud unauthenticated-mode warning | Config | Yes |
| GIMLE-333 | gimle-muninn | mTLS transport mode | Config | Yes |
| GIMLE-334 | gimle-muninn | Zero-downtime TLS material reload on certificate rotation | Config | Yes |
| GIMLE-355 | gimle-observability | Muninn endpoint list parsing from config | Config | None |
| GIMLE-264 | gimle-controlplane | CONFIG/SECRET resource-kind separation on one underlying store | Config / Authorization | Yes |
| GIMLE-038 | gimle-core | Tenant-scoped config/secret entry model | Config / Secrets | None |
| GIMLE-125 | gimle-agent | SWIM gossip membership integration with service catalog relay | Fabric | None |
| GIMLE-131 | gimle-agent | Whitelisted control-plane read relay (worker→agent→control plane) with | Fabric / Config | Yes |
| GIMLE-095 | gimle-worker | Control-plane read relay for hosted modules (RelayControlPlaneRead/Res | Fabric / Internal-Infra | Yes |
| GIMLE-126 | gimle-agent | Gossip membership read-only HTTP surface | Fabric / Observability | Yes |
| GIMLE-356 | gimle-gateway | Fabric-route HTTP-to-service dispatch | Gateway/Routing | Yes |
| GIMLE-357 | gimle-gateway | Fabric-route argument coercion (`ParamType`) | Gateway/Routing | Yes |
| GIMLE-358 | gimle-gateway | Vessel-route HTTP reverse-proxy dispatch | Gateway/Routing | Yes |
| GIMLE-359 | gimle-gateway | Vessel-endpoint resolution with TTL cache | Gateway/Routing | Yes |
| GIMLE-360 | gimle-gateway | Round-robin load balancing over ready vessel endpoints | Gateway/Routing | Yes |
| GIMLE-361 | gimle-gateway | Stale-cache fallback on endpoint-refresh failure | Gateway/Routing | Yes |
| GIMLE-362 | gimle-gateway | Vessel-route error surfacing (no ready endpoint / connect failure) | Gateway/Routing | Yes |
| GIMLE-365 | gimle-gateway | Gateway HTTP server bootstrap via module lifecycle hooks | Gateway/Routing | None |
| GIMLE-366 | gimle-gateway | Gateway liveness and readiness probes | Gateway/Routing | None |
| GIMLE-367 | gimle-gateway | HTTP status-code error mapping across the dispatcher | Gateway/Routing | Yes |
| GIMLE-369 | gimle-gateway | Vessel proxy: no TLS, no header forwarding (v1 scope limitation) | Gateway/Routing | Yes |
| GIMLE-370 | gimle-gateway | Fabric route "quiet success" ambiguity for a misrouted service name | Gateway/Routing | Yes |
| GIMLE-200 | gimle-fabric | SWIM Gossip Membership Protocol (Ping/PingReq/Ack) | Gossip Membership | Yes |
| GIMLE-201 | gimle-fabric | SWIM Self-Refutation via Incarnation Bump | Gossip Membership | Yes |
| GIMLE-202 | gimle-fabric | Lifeguard-Style Local Health Multiplier | Gossip Membership | Yes |
| GIMLE-203 | gimle-fabric | Round-Robin Bounded-Coverage Probe Target Selection | Gossip Membership | Yes |
| GIMLE-204 | gimle-fabric | Anti-Entropy Full-State Sync | Gossip Membership | Yes |
| GIMLE-205 | gimle-fabric | Dead-Member Reaping | Gossip Membership | Yes |
| GIMLE-206 | gimle-fabric | Gossip over Mutual DTLS with Deterministic Initiator Selection | Gossip Membership | Yes |
| GIMLE-090 | gimle-worker | Readiness-driven service registry availability (without restart) | Health / Fabric | Yes |
| GIMLE-121 | gimle-agent | Vessel health probing (process-alive + TCP/HTTP rungs, initial-delay a | Health / Self-Healing | None |
| GIMLE-088 | gimle-worker | Liveness/readiness probe loop with timeout and initial-delay | Health / Self-Healing | Yes |
| GIMLE-124 | gimle-agent | Periodic certificate rotation check and hot-swap of outbound HttpClien | Internal-Infra | None |
| GIMLE-135 | gimle-agent | `module-info.java` wiring for the node agent module | Internal-Infra | None |
| GIMLE-315 | gimle-andvari | mTLS server with dynamic TLS reload | Internal-Infra | Yes |
| GIMLE-317 | gimle-andvari | Andvari observability instrumentation and Muninn shipping | Internal-Infra | Yes |
| GIMLE-389 | gimle-cli | kubectl-shaped global flag parsing, manifest-kind apply dispatch, and  | Internal-Infra | Yes |
| GIMLE-243 | gimle-controlplane | Independent-executor ticking (lease/reconcile/cert-rotation isolation) | Internal-Infra | Yes |
| GIMLE-244 | gimle-controlplane | JPMS module boundary for gimle-controlplane | Internal-Infra | None |
| GIMLE-260 | gimle-controlplane | Certificate rotation (self-rotation and subject-preserving renewal) | Internal-Infra | Yes |
| GIMLE-261 | gimle-controlplane | Zero-downtime TLS material reload | Internal-Infra | Yes |
| GIMLE-267 | gimle-controlplane | `/logs/*` proxy with Muninn fallback | Internal-Infra | Yes |
| GIMLE-268 | gimle-controlplane | `/metrics-history/*` and `/traces-history/*` Muninn proxy | Internal-Infra | Yes |
| GIMLE-273 | gimle-controlplane | Per-endpoint request metrics instrumentation | Internal-Infra | Yes |
| GIMLE-289 | gimle-fafnir | mTLS HTTP server with dynamic TLS material reload | Internal-Infra | Yes |
| GIMLE-295 | gimle-fafnir | Fafnir-metrics observability instrumentation | Internal-Infra | Yes |
| GIMLE-296 | gimle-fafnir | JPMS module boundary for gimle-fafnir | Internal-Infra | None |
| GIMLE-169 | gimle-mimir | RBAC Authorization Engine | Internal-Infra | Yes |
| GIMLE-170 | gimle-mimir | Node-Tenant Assignment Check | Internal-Infra | Yes |
| GIMLE-080 | gimle-worker | Newline-delimited control-channel wire protocol (worker side) | Internal-Infra | Yes |
| GIMLE-099 | gimle-worker | `module-info.java` platform-layer/observability/fabric wiring for the  | Internal-Infra | None |
| GIMLE-270 | gimle-controlplane | Unified `AssignedInstance` wire shape across every workload kind | Internal-Infra / API Server | Yes |
| GIMLE-258 | gimle-controlplane | Bootstrap node join via single-use token + CSR | Internal-Infra / API Server (PKI) | Yes |
| GIMLE-259 | gimle-controlplane | Operator-approval-gated CSR flow | Internal-Infra / API Server (PKI) | Yes |
| GIMLE-313 | gimle-andvari | Dual audit logging for push/delete decisions | Internal-Infra / Authorization | Yes |
| GIMLE-288 | gimle-fafnir | Three-tier principal resolution (forwarded header > peer cert > sessio | Internal-Infra / Authorization | Yes |
| GIMLE-316 | gimle-andvari | Plaintext-mode loud supply-chain warning | Internal-Infra / Config | None |
| GIMLE-294 | gimle-fafnir | Muninn metrics/traces shipping | Internal-Infra / Config | None |
| GIMLE-495 | gimle-saga | Bundled console static serving | Internal-Infra / Config | Yes |
| GIMLE-094 | gimle-worker | Fabric TLS certificate rotation detection (mtime polling) | Internal-Infra / Fabric | Yes |
| GIMLE-494 | gimle-saga | Path traversal protection on run IDs | Internal-Infra / Security | Yes |
| GIMLE-010 | gimle-core | Artifact-registry vs local-path reference resolution | Internal/Infra | None |
| GIMLE-027 | gimle-core | Startup banner rendering with terminal color/Unicode auto-detection | Internal/Infra | Yes |
| GIMLE-028 | gimle-core | Single-write length-prefixed wire framing | Internal/Infra | None |
| GIMLE-029 | gimle-core | Hand-rolled JSON parser/writer | Internal/Infra | Yes |
| GIMLE-197 | gimle-fabric | Fabric Call Timeout Enforcement | Internal/Infra | Yes |
| GIMLE-198 | gimle-fabric | Fabric Frame Wire Codec | Internal/Infra | Yes |
| GIMLE-199 | gimle-fabric | Cross-JVM Object Marshalling | Internal/Infra | None |
| GIMLE-207 | gimle-fabric | SWIM Wire Codec | Internal/Infra | Yes |
| GIMLE-208 | gimle-fabric | Service Catalog Delta Wire Codec | Internal/Infra | Yes |
| GIMLE-209 | gimle-fabric | Reflective Cross-Module Method Dispatch | Internal/Infra | Yes |
| GIMLE-210 | gimle-fabric | module-info JPMS Boundary for gimle-fabric | Internal/Infra | None |
| GIMLE-368 | gimle-gateway | Boot-only platform-layer JPMS workaround (`requires static`) | Internal/Infra | Yes |
| GIMLE-414 | gimle-hilmir | Bundled JRE resolution for platform-binary launches | Internal/Infra | Yes |
| GIMLE-415 | gimle-hilmir | `java @argfile` command-line rewriting | Internal/Infra | Yes |
| GIMLE-416 | gimle-hilmir | Run ledger persistence for `up`/`down`/`status`/`upgrade-cluster` | Internal/Infra | Yes |
| GIMLE-417 | gimle-hilmir | TCP-connect readiness polling | Internal/Infra | Yes |
| GIMLE-529 | gimle-holmgang | Declarative cluster topology DSL/YAML parsing and validation | Internal/Infra | Yes |
| GIMLE-530 | gimle-holmgang | Real subprocess cluster orchestration (`GimleCluster`) | Internal/Infra | Yes |
| GIMLE-531 | gimle-holmgang | Cluster pooling per topology with destructive-scenario isolation | Internal/Infra | Yes |
| GIMLE-532 | gimle-holmgang | JUnit `@Holmgang`/`@HolmgangCluster` extension for plain-JUnit cluster | Internal/Infra | Yes |
| GIMLE-551 | gimle-holmgang | Saga unified run reporting (Gherkin + JUnit + Fenrir + Surtr) | Internal/Infra | Yes |
| GIMLE-552 | gimle-holmgang | Saga best-effort shipping to a remote report server | Internal/Infra | None |
| GIMLE-558 | gimle-holmgang | Utgard Docker container fleet management primitives | Internal/Infra | Yes |
| GIMLE-431 | gimle-maven-plugin | Internal — Aether-based cross-module runtime classpath resolution | Internal/Infra | None |
| GIMLE-432 | gimle-maven-plugin | Internal — host-matching java/mvn executable resolution and subprocess | Internal/Infra | Yes |
| GIMLE-433 | gimle-maven-plugin | Internal — git commit/branch capture for run identification | Internal/Infra | None |
| GIMLE-434 | gimle-maven-plugin | Internal — surefire report discovery and totals aggregation, including | Internal/Infra | Yes |
| GIMLE-150 | gimle-mimir | Raft RPC Wire Codec | Internal/Infra | Yes |
| GIMLE-151 | gimle-mimir | Atomic Durable File Writes | Internal/Infra | Yes |
| GIMLE-154 | gimle-mimir | Replicated Mutation Catalog (StateMutation) | Internal/Infra | Yes |
| GIMLE-166 | gimle-mimir | Store Node Leader-Only Write Gating | Internal/Infra | Yes |
| GIMLE-167 | gimle-mimir | Store Client Connection Timeout Bounds | Internal/Infra | Yes |
| GIMLE-168 | gimle-mimir | Store RPC Wire Codec | Internal/Infra | Yes |
| GIMLE-177 | gimle-mimir | Shared Domain Binary Codec | Internal/Infra | Yes |
| GIMLE-178 | gimle-mimir | Store Process Bootstrap with TLS Rotation Ticker | Internal/Infra | None |
| GIMLE-179 | gimle-mimir | Store/Raft Metrics Instrumentation | Internal/Infra | None |
| GIMLE-180 | gimle-mimir | module-info JPMS Boundary for gimle-mimir | Internal/Infra | None |
| GIMLE-327 | gimle-muninn | Day-bucketed JSON-lines store with oldest-first cursor semantics | Internal/Infra | Yes |
| GIMLE-328 | gimle-muninn | All-or-nothing batch validation on ingest | Internal/Infra | Yes |
| GIMLE-329 | gimle-muninn | Windows-safe on-disk path sanitization for colon-bearing processId | Internal/Infra | Yes |
| GIMLE-330 | gimle-muninn | Path-segment validation / directory-traversal defense | Internal/Infra | Yes |
| GIMLE-335 | gimle-muninn | Node-identity check on node-log ingest | Internal/Infra | None |
| GIMLE-336 | gimle-muninn | Instance-owner check on instance-log ingest | Internal/Infra | None |
| GIMLE-337 | gimle-muninn | Verified-certificate-presence check on metrics/traces ingest | Internal/Infra | None |
| GIMLE-338 | gimle-muninn | Read surface has no RBAC/authorization re-check (documented-vs-actual  | Internal/Infra | None |
| GIMLE-346 | gimle-observability | Multi-endpoint best-effort fan-out shipping | Internal/Infra | Yes |
| GIMLE-347 | gimle-observability | In-memory (non-persisted) log-shipping cursor | Internal/Infra | None |
| GIMLE-348 | gimle-observability | Micrometer meter → NDJSON codec | Internal/Infra | Yes |
| GIMLE-349 | gimle-observability | OpenTelemetry span → NDJSON codec | Internal/Infra | Yes |
| GIMLE-507 | gimle-smoke-tests | Real multi-process cluster fixture (store/control-plane/agent/Fafnir/M | Internal/Infra | Yes |
| GIMLE-508 | gimle-smoke-tests | On-the-fly compiled module variants via `TestModuleBuilder` | Internal/Infra | Yes |
| GIMLE-496 | gimle-testkit | Poll-until-condition primitive (`Await`) | Internal/Infra | None |
| GIMLE-497 | gimle-testkit | Kernel-assigned loopback port leasing (`PortLease`) | Internal/Infra | Yes |
| GIMLE-030 | gimle-core | Agent↔worker control-channel protocol and codec | Internal/Infra / Protocol | Yes |
| GIMLE-031 | gimle-core | Node registration/heartbeat/capacity-reporting protocol | Internal/Infra / Protocol | None |
| GIMLE-041 | gimle-core | Saga test-run event model and NDJSON codec | Internal/Infra / Testing | Yes |
| GIMLE-042 | gimle-core | Stable failure-signature hashing for flaky-test clustering | Internal/Infra / Testing | Yes |
| GIMLE-039 | gimle-core | Bundled SPA static-asset resolution from classpath | Internal/Infra / Web | Yes |
| GIMLE-040 | gimle-core | SPA static file serving with client-side-route fallback | Internal/Infra / Web | Yes |
| GIMLE-184 | gimle-fabric | Locality-Aware Load Balancing with Spillover | Load Balancing | Yes |
| GIMLE-185 | gimle-fabric | Least-Outstanding-Requests Selection | Load Balancing | Yes |
| GIMLE-548 | gimle-holmgang | Surtr scale/churn/performance workload runner | Load Testing | Yes |
| GIMLE-549 | gimle-holmgang | Surtr Muninn-window measurement (documented gap) | Load Testing | None |
| GIMLE-550 | gimle-holmgang | Module-density Tier 1 packing Surtr reference workload | Load Testing | Yes |
| GIMLE-521 | gimle-smoke-tests | Autoscaling under real request-rate, error-rate, queue-depth, and weig | Load Testing / Cluster Validation | Yes |
| GIMLE-319 | gimle-muninn | Node platform-log ingest | Logging | Yes |
| GIMLE-320 | gimle-muninn | Instance-log ingest | Logging | Yes |
| GIMLE-321 | gimle-muninn | Node/instance log read with cursor paging | Logging | Yes |
| GIMLE-322 | gimle-muninn | `follow=true` rejection on Muninn reads | Logging | Yes |
| GIMLE-343 | gimle-observability | Periodic log-file shipping to Muninn | Logging | Yes |
| GIMLE-323 | gimle-muninn | Metrics ingest | Metrics | Yes |
| GIMLE-324 | gimle-muninn | Metrics read | Metrics | Yes |
| GIMLE-352 | gimle-observability | Per-process tagged Micrometer metrics wrappers | Metrics | Yes |
| GIMLE-353 | gimle-observability | WorkerMetrics thread-count / metaspace gauges | Metrics | Yes |
| GIMLE-354 | gimle-observability | Fafnir authz-failure counter (rate-limiting signal) | Metrics | Yes |
| GIMLE-001 | gimle-core | Semantic module versioning | Module System | Yes |
| GIMLE-002 | gimle-core | Version range constraint matching | Module System | Yes |
| GIMLE-003 | gimle-core | Module descriptor validation (request ≤ limit invariant) | Module System | Yes |
| GIMLE-004 | gimle-core | Tiered isolation model (TIER_1/TIER_2/TIER_3) | Module System | None |
| GIMLE-043 | gimle-module | Module dependency resolution with cycle detection | Module System | Yes |
| GIMLE-044 | gimle-module | Module registry (install bookkeeping, idempotent re-install, content-m | Module System | Yes |
| GIMLE-045 | gimle-module | Module lifecycle state machine (INSTALLED→RESOLVED→STARTING→ACTIVE→STO | Module System | Yes |
| GIMLE-046 | gimle-module | Dynamic per-module-version JPMS ModuleLayer construction | Module System | Yes |
| GIMLE-049 | gimle-module | Repeated-redeploy flat-metaspace acceptance test | Module System | Yes |
| GIMLE-051 | gimle-module | Module lifecycle hooks (reflectively instantiated, JPMS-exported) | Module System | Yes |
| GIMLE-052 | gimle-module | Job-kind run-to-completion hooks | Module System | Yes |
| GIMLE-053 | gimle-module | Module context API (in-flight tracking, service lookup, config, data d | Module System | Yes |
| GIMLE-054 | gimle-module | In-worker round-robin service registry with version-aware cutover | Module System | Yes |
| GIMLE-055 | gimle-module | Cross-tier name-driven service invocation | Module System | Yes |
| GIMLE-056 | gimle-module | Same-worker cross-module service publish/discover | Module System | Yes |
| GIMLE-058 | gimle-module | Hot redeploy (old/new version coexistence with pinned dependent wiring | Module System | Yes |
| GIMLE-059 | gimle-module | gimle-module.yaml descriptor parsing and validation | Module System | Yes |
| GIMLE-060 | gimle-module | Module artifact reading — real-JPMS-module and descriptor-presence val | Module System | Yes |
| GIMLE-081 | gimle-worker | Module install/resolve/start/stop/uninstall command dispatch | Module System | None |
| GIMLE-082 | gimle-worker | Instance identity registration and rename-in-place | Module System | None |
| GIMLE-085 | gimle-worker | Classloader leak detection on undeploy | Module System | None |
| GIMLE-092 | gimle-worker | Job-kind module execution (run-to-completion, not probed) | Module System | Yes |
| GIMLE-008 | gimle-core | Health probe configuration with initial delay | Module System / Health | Yes |
| GIMLE-063 | gimle-module | Health probe interfaces (liveness/readiness) | Module System / Health | None |
| GIMLE-047 | gimle-module | Unnamed-module readability grant for bundled hooks/probes | Module System / Internal-Infra | Yes |
| GIMLE-048 | gimle-module | Classloader leak detection via PhantomReference | Module System / Internal-Infra | Yes |
| GIMLE-050 | gimle-module | Best-effort leak retaining-path attribution via JFR OldObjectSample | Module System / Internal-Infra | Yes |
| GIMLE-062 | gimle-module | Multi-endpoint Andvari failover on pull | Module System / Internal-Infra | None |
| GIMLE-100 | gimle-worker | Real bundled-hook/probe classloading against the platform layer | Module System / Internal-Infra | Yes |
| GIMLE-006 | gimle-core | Tenant-scoped service export | Module System / Multi-tenancy | Yes |
| GIMLE-057 | gimle-module | Graceful drain-then-dispose stop with deadline | Module System / Self-Healing | Yes |
| GIMLE-007 | gimle-core | StatefulSet-shaped persistent volume declaration | Module System / Storage | Yes |
| GIMLE-009 | gimle-core | Vessel hosting mode (plain-process workload) | Module System / Vessel Hosting | Yes |
| GIMLE-037 | gimle-core | Tenant identity and resource quota model | Multi-tenancy | None |
| GIMLE-271 | gimle-controlplane | Reserved system-tenant auto-seeding | Multi-tenancy / Internal-Infra | Yes |
| GIMLE-127 | gimle-agent | Node/instance log-serving HTTP surface with tailing and follow | Observability | Yes |
| GIMLE-128 | gimle-agent | Merged node-level SYSTEM log view | Observability | None |
| GIMLE-130 | gimle-agent | Node-agent log/metrics shipping to Muninn (own + supervised) | Observability | Yes |
| GIMLE-133 | gimle-agent | Instance-event forwarding (worker-reported and agent-originated) to co | Observability | None |
| GIMLE-032 | gimle-core | Instance lifecycle event log model | Observability | None |
| GIMLE-331 | gimle-muninn | Age-based retention sweep | Observability | Yes |
| GIMLE-339 | gimle-muninn | `/status` operational endpoint | Observability | Yes |
| GIMLE-351 | gimle-observability | JFR-based per-module CPU/allocation attribution | Observability | Yes |
| GIMLE-084 | gimle-worker | Durable InstanceEvent emission per lifecycle transition | Observability | None |
| GIMLE-087 | gimle-worker | OpenTelemetry context propagation across virtual-thread dispatch | Observability | Yes |
| GIMLE-129 | gimle-agent | `hs_err_pid*.log` crash-dump listing and fetch | Observability / Cgroup Management | Yes |
| GIMLE-097 | gimle-worker | Per-module CPU/memory/request-rate/error-rate metrics reporting (porta | Observability / Cgroup Management | None |
| GIMLE-105 | gimle-agent | Worker stdout draining, JSON-line de-duplication, and raw SYSTEM-line  | Observability / Internal-Infra | Yes |
| GIMLE-083 | gimle-worker | Per-instance MDC log tagging for lifecycle/hook/probe/request-dispatch | Observability / Internal-Infra | Yes |
| GIMLE-096 | gimle-worker | Worker-side trace relay to agent (no direct Muninn shipping) | Observability / Internal-Infra | Yes |
| GIMLE-098 | gimle-worker | Worker-wide meter snapshot relay to Muninn (via agent) | Observability / Internal-Infra | None |
| GIMLE-019 | gimle-core | Structured JSON log encoding with APPLICATION/PLATFORM categorization | Observability / Logging | Yes |
| GIMLE-020 | gimle-core | Human-readable colored console log encoding | Observability / Logging | Yes |
| GIMLE-021 | gimle-core | Runtime-switchable console log format (text default, JSON opt-in) | Observability / Logging | Yes |
| GIMLE-022 | gimle-core | MDC-tagged proxying for same-worker and probe-loop invocations | Observability / Logging | Yes |
| GIMLE-023 | gimle-core | Per-instance sifted log files | Observability / Logging | Yes |
| GIMLE-024 | gimle-core | Platform (non-instance) log file appender | Observability / Logging | Yes |
| GIMLE-025 | gimle-core | Kubelet-style size/count log rotation | Observability / Logging | Yes |
| GIMLE-026 | gimle-core | Cursor-based log paging and live-follow streaming | Observability / Logging | Yes |
| GIMLE-113 | gimle-agent | Worker-crash-to-durable-InstanceEvent relay | Observability / Self-Healing | None |
| GIMLE-132 | gimle-agent | Node capacity/instance-observation heartbeat reporting | Observability / Worker Supervision | Yes |
| GIMLE-242 | gimle-controlplane | Reconciler-leader election via non-replicated lease | Orchestration / Internal-Infra | Yes |
| GIMLE-034 | gimle-core | Certificate bootstrap (CSR) request/response protocol | PKI | None |
| GIMLE-070 | gimle-pki | Self-signed cluster CA generation | PKI | Yes |
| GIMLE-071 | gimle-pki | CSR-to-leaf-certificate signing with signature verification | PKI | Yes |
| GIMLE-073 | gimle-pki | CSR generation with Subject Alternative Names | PKI | Yes |
| GIMLE-075 | gimle-pki | Randomized certificate-renewal scheduling (anti-thundering-herd) | PKI | None |
| GIMLE-076 | gimle-pki | Own-certificate rotation over mTLS via CSR bootstrap endpoint | PKI | None |
| GIMLE-074 | gimle-pki | Hand-rolled PEM encode/decode for certs, CSRs, and private keys | PKI / Internal-Infra | Yes |
| GIMLE-078 | gimle-pki | Cluster PKI bootstrap CLI (`mvn gimle:tls-init`) | PKI / Internal-Infra | None |
| GIMLE-072 | gimle-pki | Server-stamped Subject override on signing (prevents self-declared pri | PKI / Security | Yes |
| GIMLE-077 | gimle-pki | X.500 Subject utilities: server-side O= stamping and Principal derivat | PKI / Security | Yes |
| GIMLE-560 | gimle-dist | Standalone CLI distribution archive | Packaging | None |
| GIMLE-561 | gimle-dist | Standalone Hilmir bootstrap-tool distribution archive | Packaging | None |
| GIMLE-562 | gimle-dist | Cluster-machine platform distribution archive | Packaging | None |
| GIMLE-563 | gimle-dist | Opt-in bundled-JRE distribution variant (`dist-with-jre` profile) | Packaging | None |
| GIMLE-564 | gimle-dist | Distribution archive checksums and SBOM generation | Packaging | None |
| GIMLE-559 | gimle-holmgang | Docker Compose manual validation topologies (bundled-JRE and full-JRE) | Packaging / Internal-Infra | None |
| GIMLE-138 | gimle-mimir | Election Safety Restriction (log up-to-date check) | Raft Consensus | Yes |
| GIMLE-139 | gimle-mimir | Conflicting-Entry Truncation | Raft Consensus | Yes |
| GIMLE-140 | gimle-mimir | Leader-Only-Commits-Own-Term Rule (Figure 8) | Raft Consensus | Yes |
| GIMLE-142 | gimle-mimir | Proposal Timeout with Ghost-Write Prevention | Raft Consensus | Yes |
| GIMLE-143 | gimle-mimir | Chunked InstallSnapshot Transfer (Figure 13) | Raft Consensus | Yes |
| GIMLE-144 | gimle-mimir | Local Log Compaction / Snapshotting | Raft Consensus | Yes |
| GIMLE-147 | gimle-mimir | Non-Voting Learner & Automatic Promotion | Raft Consensus | Yes |
| GIMLE-149 | gimle-mimir | Raft Transport over Mutual TLS with Hot Cert Reload | Raft Consensus | Yes |
| GIMLE-565 | gimle-mimir | Norn deterministic virtual-time Raft fault-injection simulation | Raft Consensus / Internal-Infra / Testing | Yes |
| GIMLE-220 | gimle-controlplane | Deployment scale-down | Reconciliation | Yes |
| GIMLE-227 | gimle-controlplane | Readiness-only failures never trigger reschedule | Reconciliation | Yes |
| GIMLE-240 | gimle-controlplane | CronJob missed-schedule starting-deadline handling | Reconciliation | Yes |
| GIMLE-239 | gimle-controlplane | CronJob manual trigger (`gimle cronjob trigger`) | Reconciliation / API Server | Yes |
| GIMLE-221 | gimle-controlplane | Artifact-hash drift detection at reconcile time | Reconciliation / Internal-Infra | Yes |
| GIMLE-225 | gimle-controlplane | Persisted grace-period bookkeeping (survives leader failover) | Reconciliation / Internal-Infra | Yes |
| GIMLE-231 | gimle-controlplane | DaemonSet reconciliation and rolling update | Reconciliation / Orchestration | Yes |
| GIMLE-233 | gimle-controlplane | StatefulSet OrderedReady placement | Reconciliation / Orchestration | Yes |
| GIMLE-234 | gimle-controlplane | StatefulSet one-index-at-a-time scale-down | Reconciliation / Orchestration | Yes |
| GIMLE-235 | gimle-controlplane | JobRun run-to-completion reconciliation | Reconciliation / Orchestration | Yes |
| GIMLE-236 | gimle-controlplane | Job active-deadline enforcement | Reconciliation / Orchestration | Yes |
| GIMLE-237 | gimle-controlplane | CronJob schedule-driven Job materialization | Reconciliation / Orchestration | Yes |
| GIMLE-238 | gimle-controlplane | CronJob concurrency policy (Allow/Forbid/Replace) | Reconciliation / Orchestration | Yes |
| GIMLE-230 | gimle-controlplane | Autoscaling WEIGHTED combination mode | Reconciliation / Scheduling | Yes |
| GIMLE-224 | gimle-controlplane | Node-death instance reclamation (`ReplicaCountReconciler`) | Reconciliation / Self-healing | Yes |
| GIMLE-226 | gimle-controlplane | Unhealthy-instance backoff-gated reschedule (`HealthReconciler`) | Reconciliation / Self-healing | Yes |
| GIMLE-232 | gimle-controlplane | DaemonSet dark-node placement-safety grace period | Reconciliation / Self-healing | Yes |
| GIMLE-390 | gimle-hilmir | Topology validation (`hilmir validate`) | Release Management | Yes |
| GIMLE-391 | gimle-hilmir | Cluster launch planning (`hilmir plan`) | Release Management | Yes |
| GIMLE-392 | gimle-hilmir | Real multi-process cluster bring-up (`hilmir up`) | Release Management | Yes |
| GIMLE-393 | gimle-hilmir | Cluster teardown and status reporting (`hilmir down`/`status`) | Release Management | Yes |
| GIMLE-395 | gimle-hilmir | Raft store membership add (`hilmir store add`) | Release Management | Yes |
| GIMLE-396 | gimle-hilmir | Raft store membership remove (`hilmir store remove`) | Release Management | Yes |
| GIMLE-397 | gimle-hilmir | Per-machine platform binary rolling upgrade with quorum-safe store res | Release Management | Yes |
| GIMLE-398 | gimle-hilmir | Bundle-based fresh release deployment (`hilmir deploy`) | Release Management | Yes |
| GIMLE-399 | gimle-hilmir | Bundle upgrade with automatic resource pruning (`hilmir upgrade`) | Release Management | Yes |
| GIMLE-400 | gimle-hilmir | Release rollback to a prior revision (`hilmir rollback`) | Release Management | Yes |
| GIMLE-401 | gimle-hilmir | Full release teardown (`hilmir undeploy`) | Release Management | Yes |
| GIMLE-402 | gimle-hilmir | Release listing (`hilmir releases`) | Release Management | Yes |
| GIMLE-403 | gimle-hilmir | Release status inspection (`hilmir release-status`) | Release Management | Yes |
| GIMLE-404 | gimle-hilmir | GitOps directory reconciliation (`hilmir sync`, incl. `--watch` and `- | Release Management | Yes |
| GIMLE-405 | gimle-hilmir | `--watch` interval loop for sync | Release Management | None |
| GIMLE-406 | gimle-hilmir | Bundle value templating and override precedence (`${values.*}` substit | Release Management | Yes |
| GIMLE-407 | gimle-hilmir | Bundle manifest schema parsing and validation | Release Management | Yes |
| GIMLE-408 | gimle-hilmir | Workload readiness polling for `--wait` | Release Management | Yes |
| GIMLE-412 | gimle-hilmir | Gateway extension enable (`hilmir enable gateway`) | Release Management | Yes |
| GIMLE-413 | gimle-hilmir | Gateway extension disable (`hilmir disable gateway`) | Release Management | Yes |
| GIMLE-394 | gimle-hilmir | Cluster TLS/PKI bootstrap (`hilmir pki init`) | Release Management / Security | Yes |
| GIMLE-482 | gimle-saga | NDJSON event ingest API | Reporting backend / Internal-Infra | Yes |
| GIMLE-483 | gimle-saga | Idempotent per-run ingest / re-ingest replacement | Reporting backend / Internal-Infra | Yes |
| GIMLE-484 | gimle-saga | Crash-safe append (torn-tail recovery) | Reporting backend / Internal-Infra | Yes |
| GIMLE-485 | gimle-saga | Surefire/Failsafe XML import | Reporting backend / Internal-Infra | Yes |
| GIMLE-486 | gimle-saga | Fold-import safety net for a live run's gap | Reporting backend / Internal-Infra | Yes |
| GIMLE-487 | gimle-saga | Run listing, detail, and cursor-paginated event reads | Reporting backend / Internal-Infra | Yes |
| GIMLE-488 | gimle-saga | Live NDJSON tail (`follow=true`) of a run's event stream | Reporting backend / Internal-Infra | Yes |
| GIMLE-489 | gimle-saga | Abandoned-run detection on restart | Reporting backend / Internal-Infra | Yes |
| GIMLE-490 | gimle-saga | Flake ledger derivation (fail-then-pass rule) and rebuild | Reporting backend / Internal-Infra | Yes |
| GIMLE-491 | gimle-saga | Flaky scoreboard with time-window ranking | Reporting backend / Internal-Infra | Yes |
| GIMLE-492 | gimle-saga | Test-tag index and quarantine status | Reporting backend / Internal-Infra | Yes |
| GIMLE-493 | gimle-saga | Per-test history endpoint | Reporting backend / Internal-Infra | Yes |
| GIMLE-005 | gimle-core | Kubernetes-shaped resource quantity parsing | Resource Limiting | Yes |
| GIMLE-064 | gimle-os | Pluggable resource-limiter abstraction | Resource Limiting | Yes |
| GIMLE-065 | gimle-os | Portable JVM-flags resource enforcement (Tier 1/Tier 2) | Resource Limiting | Yes |
| GIMLE-066 | gimle-os | Tier 3 (namespace isolation) — deliberately unsupported by the current | Resource Limiting | Yes |
| GIMLE-067 | gimle-os | Kernel-level (cgroup v2) resource enforcement — deferred | Resource Limiting | None |
| GIMLE-503 | gimle-examples | `hello-module` — minimal inert deployable fixture | Sample Module | None |
| GIMLE-211 | gimle-controlplane | First-fit-decreasing bin-packing scheduler | Scheduling | Yes |
| GIMLE-212 | gimle-controlplane | Isolation-tier placement filtering | Scheduling | Yes |
| GIMLE-214 | gimle-controlplane | Strict anti-affinity across nodes | Scheduling | Yes |
| GIMLE-216 | gimle-controlplane | Required node-label placement constraint | Scheduling | Yes |
| GIMLE-218 | gimle-controlplane | DaemonSet eligible-node enumeration (`eligibleNodes`) | Scheduling | Yes |
| GIMLE-035 | gimle-core | Assigned-instance work-order model (incl. in-place rename and vessel d | Scheduling | None |
| GIMLE-215 | gimle-controlplane | Tier 2/3 node-level tenant isolation | Scheduling / Multi-tenancy | Yes |
| GIMLE-217 | gimle-controlplane | StatefulSet sticky node placement | Scheduling / Orchestration | Yes |
| GIMLE-263 | gimle-controlplane | Secrets key rotation trigger (proxied) | Secrets Management | Yes |
| GIMLE-276 | gimle-fafnir | AES-256-GCM secret value encryption with versioned key IDs | Secrets Management | Yes |
| GIMLE-277 | gimle-fafnir | Legacy pre-key-id ciphertext format fallback | Secrets Management | Yes |
| GIMLE-278 | gimle-fafnir | Local AES-256 key-file generation and loading | Secrets Management | Yes |
| GIMLE-279 | gimle-fafnir | Key rotation with full-ring persistence (`KeyFileManager.rotate`) | Secrets Management | Yes |
| GIMLE-281 | gimle-fafnir | Full-key-rotation re-encryption sweep | Secrets Management | Yes |
| GIMLE-282 | gimle-fafnir | Versioned secret storage layered over ConfigEntry | Secrets Management | Yes |
| GIMLE-284 | gimle-fafnir | Soft delete vs hard delete (`?destroy=true`) | Secrets Management | Yes |
| GIMLE-262 | gimle-controlplane | `/secrets/*` byte-for-byte proxy to Fafnir | Secrets Management / Internal-Infra | Yes |
| GIMLE-280 | gimle-fafnir | Key-ring fingerprinting for cross-replica drift detection | Secrets Management / Internal-Infra | Yes |
| GIMLE-283 | gimle-fafnir | Optimistic-write versioned put with narrow-lease serialization | Secrets Management / Internal-Infra | Yes |
| GIMLE-013 | gimle-core | Console password hashing (PBKDF2-HMAC-SHA256) | Security | Yes |
| GIMLE-016 | gimle-core | Stateless HMAC-signed console session tokens | Security | Yes |
| GIMLE-017 | gimle-core | Session-signing key file load-or-create with owner-only permissions | Security | Yes |
| GIMLE-018 | gimle-core | Per-key exponential-backoff login throttle | Security | Yes |
| GIMLE-033 | gimle-core | Cross-resource audit trail model | Security / Audit | Yes |
| GIMLE-011 | gimle-core | RBAC domain model (resources, verbs, permissions, roles, bindings) | Security / RBAC | Yes |
| GIMLE-012 | gimle-core | Built-in cluster-admin role and operator/node certificate groups | Security / RBAC | Yes |
| GIMLE-122 | gimle-agent | Vessel crash respawn resets probe initial-delay clock | Self-Healing | None |
| GIMLE-181 | gimle-fabric | Same-Worker Direct Invocation Tier | Service Fabric | Yes |
| GIMLE-183 | gimle-fabric | Cross-Machine TCP Invocation Tier | Service Fabric | Yes |
| GIMLE-190 | gimle-fabric | Gossip-Propagated Service Catalog | Service Fabric | Yes |
| GIMLE-191 | gimle-fabric | Catalog Eviction on Gossip-Detected Node Death | Service Fabric | Yes |
| GIMLE-192 | gimle-fabric | Cross-Tenant Service Export Access Control | Service Fabric | Yes |
| GIMLE-193 | gimle-fabric | Runtime Name-Driven Cross-Tier Invocation (invokeByName) | Service Fabric | Yes |
| GIMLE-194 | gimle-fabric | Inbound Call Dispatch with Bounded Concurrency | Service Fabric | Yes |
| GIMLE-195 | gimle-fabric | Distributed Trace Propagation Across Fabric Hops | Service Fabric | Yes |
| GIMLE-196 | gimle-fabric | Fabric Transport over Mutual TLS with Hot Cert Reload | Service Fabric | Yes |
| GIMLE-152 | gimle-mimir | File-Backed State Store Persistence Engine | State Store | Yes |
| GIMLE-153 | gimle-mimir | Full-State Snapshot / Restore | State Store | Yes |
| GIMLE-155 | gimle-mimir | Leader-Local Node Heartbeat Tracking | State Store | Yes |
| GIMLE-156 | gimle-mimir | Distributed Lease Coordination (Grant/Renew/Release) | State Store | Yes |
| GIMLE-157 | gimle-mimir | Per-Instance Lifecycle Event Log with Retention Cap | State Store | Yes |
| GIMLE-158 | gimle-mimir | Cluster-Wide Audit Trail with Filtering | State Store | Yes |
| GIMLE-160 | gimle-mimir | StatefulSet OrderedReady Index & Sticky Node Binding | State Store | None |
| GIMLE-163 | gimle-mimir | RBAC Data Persistence (Roles, RoleBindings, Accounts) | State Store | Yes |
| GIMLE-068 | gimle-os | Pluggable persistent-volume-manager abstraction | Storage | Yes |
| GIMLE-069 | gimle-os | Local-disk persistent volume allocation for StatefulSet-shaped instanc | Storage | Yes |
| GIMLE-498 | gimle-testkit | Heimdall event-driven cluster condition harness | Test Infrastructure | None |
| GIMLE-499 | gimle-testkit | Replica-scoped condition observation | Test Infrastructure | Yes |
| GIMLE-500 | gimle-testkit | Deployment/node/log condition builders | Test Infrastructure | Yes |
| GIMLE-501 | gimle-testkit | Time-windowed negative invariants (`Invariant`/`InvariantGuard`) | Test Infrastructure | Yes |
| GIMLE-502 | gimle-testkit | Forensic failure reporting | Test Infrastructure | Yes |
| GIMLE-325 | gimle-muninn | Traces ingest | Tracing | Yes |
| GIMLE-326 | gimle-muninn | Traces read | Tracing | Yes |
| GIMLE-340 | gimle-observability | Default OpenTelemetry tracer installation | Tracing | Yes |
| GIMLE-341 | gimle-observability | Configurable, batched span exporter installation | Tracing | Yes |
| GIMLE-342 | gimle-observability | Bounded-wait tracer flush | Tracing | Yes |
| GIMLE-345 | gimle-observability | One-shot trace-batch and prepared-batch shipping | Tracing | Yes |
| GIMLE-350 | gimle-observability | `MuninnSpanExporter` (OpenTelemetry SDK integration) | Tracing | Yes |
| GIMLE-467 | gimle-andvari-console | Andvari operator login/logout (session-cookie auth) | Web Console / Auth | Yes |
| GIMLE-435 | gimle-console | Operator session login / logout | Web Console / Auth | Yes |
| GIMLE-436 | gimle-console | Session bootstrap & 401 handling | Web Console / Auth | Yes |
| GIMLE-461 | gimle-fafnir-console | Vault operator login/logout (session-cookie auth) | Web Console / Auth | Yes |
| GIMLE-468 | gimle-andvari-console | Registry status overview (uptime, transport, recent pushes) | Web Console / Frontend | Yes |
| GIMLE-469 | gimle-andvari-console | Artifact catalog browsing & search | Web Console / Frontend | Yes |
| GIMLE-470 | gimle-andvari-console | Artifact version detail (download, checksum display, delete) | Web Console / Frontend | Yes |
| GIMLE-471 | gimle-andvari-console | Client-side SHA-256 checksum verification on download | Web Console / Frontend | Yes |
| GIMLE-472 | gimle-andvari-console | Push artifact dialog (drag-and-drop upload) | Web Console / Frontend | Yes |
| GIMLE-473 | gimle-andvari-console | Maven-2 repository interop view | Web Console / Frontend | None |
| GIMLE-474 | gimle-andvari-console | Andvari copy-to-clipboard utility | Web Console / Frontend | None |
| GIMLE-437 | gimle-console | Cluster Overview dashboard | Web Console / Frontend | None |
| GIMLE-438 | gimle-console | Tactical HUD / Signal display-mode toggle | Web Console / Frontend | None |
| GIMLE-439 | gimle-console | Deployments list/create/detail/delete | Web Console / Frontend | Yes |
| GIMLE-440 | gimle-console | Jobs (run-to-completion workload) list | Web Console / Frontend | Yes |
| GIMLE-441 | gimle-console | CronJobs list/detail | Web Console / Frontend | Yes |
| GIMLE-442 | gimle-console | DaemonSets list/detail | Web Console / Frontend | Yes |
| GIMLE-443 | gimle-console | StatefulSets list/detail | Web Console / Frontend | Yes |
| GIMLE-444 | gimle-console | Instances table with filtering (global + node/tenant-scoped) | Web Console / Frontend | Yes |
| GIMLE-445 | gimle-console | Nodes list/detail with capacity bars and staleness | Web Console / Frontend | Yes |
| GIMLE-446 | gimle-console | Tenants list/detail with quota management and delete | Web Console / Frontend | Yes |
| GIMLE-447 | gimle-console | Topology placement map | Web Console / Frontend | None |
| GIMLE-448 | gimle-console | Cluster metrics charts (lifecycle mix, capacity, quota pressure) | Web Console / Frontend | None |
| GIMLE-449 | gimle-console | Per-process metrics history (Muninn-backed) | Web Console / Frontend | Yes |
| GIMLE-450 | gimle-console | Trace span history viewer | Web Console / Frontend | Yes |
| GIMLE-451 | gimle-console | Log explorer with live tailing | Web Console / Frontend | Yes |
| GIMLE-452 | gimle-console | Crash-dump (hs_err) listing on Logs screen | Web Console / Frontend | None |
| GIMLE-453 | gimle-console | Config entries management (per-tenant) | Web Console / Frontend | Yes |
| GIMLE-454 | gimle-console | Secrets management (Fafnir-backed, versioned) | Web Console / Frontend | Yes |
| GIMLE-455 | gimle-console | Module artifact registry browser (Andvari-backed) | Web Console / Frontend | Yes |
| GIMLE-456 | gimle-console | RBAC access control (roles, role bindings, accounts) | Web Console / Frontend | Yes |
| GIMLE-457 | gimle-console | Audit trail viewer with filtering | Web Console / Frontend | Yes |
| GIMLE-458 | gimle-console | Control-plane status panel | Web Console / Frontend | None |
| GIMLE-459 | gimle-console | Theme toggle (light/dark) | Web Console / Frontend | None |
| GIMLE-462 | gimle-fafnir-console | Vault status overview (uptime, active key, transport mode, tenants) | Web Console / Frontend | None |
| GIMLE-463 | gimle-fafnir-console | Secrets browsing/reveal/version/write/destroy (vault-native UI) | Web Console / Frontend | Yes |
| GIMLE-464 | gimle-fafnir-console | Tenant filter via URL search param | Web Console / Frontend | None |
| GIMLE-465 | gimle-fafnir-console | Key rotation trigger | Web Console / Frontend | Yes |
| GIMLE-466 | gimle-fafnir-console | Fafnir console error banner / global error capture | Web Console / Frontend | None |
| GIMLE-481 | gimle-saga-console | Saga console theming (no auth surface) | Web Console / Frontend | Yes |
| GIMLE-475 | gimle-saga-console | Runs list (no authentication) | Web Console / Reporting | Yes |
| GIMLE-476 | gimle-saga-console | Live run detail with streaming test feed | Web Console / Reporting | Yes |
| GIMLE-477 | gimle-saga-console | Run attachments: Gherkin scenario tree, Chaos ledger, Surtr phase tabl | Web Console / Reporting | Yes |
| GIMLE-478 | gimle-saga-console | Test detail / per-test history | Web Console / Reporting | Yes |
| GIMLE-479 | gimle-saga-console | Compare two runs (diff view) | Web Console / Reporting | Yes |
| GIMLE-480 | gimle-saga-console | Gjallarhorn flake scoreboard | Web Console / Reporting | Yes |
| GIMLE-460 | gimle-console | Playwright end-to-end smoke suite against a real cluster | Web Console / Testing | Yes |
| GIMLE-102 | gimle-agent | Worker JVM process spawn and command-line construction | Worker Supervision | Yes |
| GIMLE-104 | gimle-agent | Deliberate-stop suppression of crash-respawn | Worker Supervision | Yes |
| GIMLE-109 | gimle-agent | Assignment reconciliation loop (fetch, start, replace, stop) | Worker Supervision | Yes |
| GIMLE-110 | gimle-agent | Tier 1 density — shared-worker reuse for multiple module instances | Worker Supervision | Yes |
| GIMLE-111 | gimle-agent | Instance rename-in-place (no restart) | Worker Supervision | Yes |
| GIMLE-118 | gimle-agent | Vessel process supervision (plain-jar workload as its own dedicated pr | Worker Supervision | Yes |
| GIMLE-086 | gimle-worker | Per-module bounded virtual-thread scheduler | Worker Supervision | Yes |
| GIMLE-106 | gimle-agent | Machine-level capacity tracking and admission (memory/CPU) | Worker Supervision / Config | Yes |
| GIMLE-101 | gimle-agent | Node agent registration and repeating reconcile/heartbeat/rotate tick  | Worker Supervision / Internal-Infra | Yes |
| GIMLE-079 | gimle-worker | Worker JVM control-channel bootstrap | Worker Supervision / Internal-Infra | Yes |
| GIMLE-091 | gimle-worker | Stopping/Uninstalled teardown of scheduler, probes, and service regist | Worker Supervision / Module System | Yes |
| GIMLE-114 | gimle-agent | Install-phase Nack escalates to FAILED (closing the "stuck at INSTALLE | Worker Supervision / Self-Healing | None |
