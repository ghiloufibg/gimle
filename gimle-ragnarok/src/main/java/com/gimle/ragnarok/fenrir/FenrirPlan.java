  ...
  public static final class Builder {
    ...
    private void validate() {
      if (soak == null || soak.isZero() || soak.isNegative()) {
        throw new RagnarokException("a plan must set a positive soak window via soakFor(...)");
      }
      if (gapMin.isNegative() || gapMin.isZero()) {
        throw new RagnarokException("the strike gap minimum must be positive");
      }
      ...
      if (gateTimeout.isNegative() || gateTimeout.isZero()) {
        throw new RagnarokException("the recovery gate timeout must be positive");
      }
      if (pools.isEmpty()) {
        throw new RagnarokException("a plan must declare at least one pool");
      }
      final Set<FaultKind> kinds = new LinkedHashSet<>();
      for (final Pool pool : pools) {
        if (!kinds.add(pool.kind())) {
          throw new RagnarokException("duplicate pool for fault kind " + pool.kind());
        }
