...
public record SurtrWorkload(
  ...
  public SurtrWorkload {
    if (name == null || name.isBlank()) {
      throw new RagnarokException("a workload must have a name");
    }
    if (topology == null || topology.isBlank()) {
      throw new RagnarokException("a workload must name a topology");
    }
    jobs = List.copyOf(jobs);
    measurements = List.copyOf(measurements);
    if (jobs.isEmpty()) {
      throw new RagnarokException("a workload must declare at least one job");
    }
    if (gates == null) {
  ...
  public static double scaleFactor() {
    ...
    try {
      ...
      if (scale <= 0) {
        throw new RagnarokException("gimle.surtr.scale must be positive, got " + raw);
      }
      return scale;
    } catch (final NumberFormatException e) {
      throw new RagnarokException("gimle.surtr.scale must be a number, got " + raw, e);
    }
  }
