...
/**
 * The raw, unopened configuration for an {@link EndpointClusterTarget}: base URLs and store
 * endpoints exactly as an operator wrote them, plus how to reach them (plaintext or mTLS). {@link
 * EndpointClusterTarget}/{@link HttpControlPlaneClient} are themselves TLS-agnostic -- both just
 * take a pre-built {@link HttpClient} -- so {@link #open()} is the one place that turns this
 * declarative spec into a live target, the same split {@link
 * com.gimle.holmgang.cluster.GimleCluster}'s own {@code buildOperatorClient()} already draws
 * between "what to connect to" and "the client that connects".
 */
public record TargetSpec(
  ...
  /** Builds the {@link HttpClient} and constructs the live {@link EndpointClusterTarget}. */
  public EndpointClusterTarget open() {
