...
public final class AndvariMojo extends AbstractGimleMojo {
  ...
  /**
   * Andvari's own TLS leaf certificate/key and the cluster CA certificate, forwarded verbatim as
   * {@code -Dgimle.tls.certFile}/{@code keyFile}/{@code caFile} -- the exact system properties
   * {@code AndvariMain} itself reads, and the same plain (not {@code gimle.andvari}-namespaced)
   ...
   * meaningful once {@code gimle.andvari.transportProtocol=tls} is also set.
   */
