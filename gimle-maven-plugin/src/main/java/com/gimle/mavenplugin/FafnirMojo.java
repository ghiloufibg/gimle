...
public final class FafnirMojo extends AbstractGimleMojo {
  ...
  /**
   * Fafnir's own TLS leaf certificate/key and the cluster CA certificate, forwarded verbatim as
   * {@code -Dgimle.tls.certFile}/{@code keyFile}/{@code caFile} -- the exact system properties
   * {@code FafnirMain} itself reads, and the same plain (not {@code gimle.fafnir}-namespaced)
   ...
   * meaningful once {@code gimle.fafnir.transportProtocol=tls} is also set.
   */
