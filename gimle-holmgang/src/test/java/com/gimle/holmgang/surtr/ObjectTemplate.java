...
public record ObjectTemplate(
  ...
  public enum Kind {
    ...
    static Kind fromField(final String field) {
      return switch (field) {
        ...
        default -> throw new HolmgangException("unknown object kind: " + field);
      };
    }
  }
  ...
  public ObjectTemplate {
    if (kind == null) {
      throw new HolmgangException("an object template must have a kind");
    }
    if (name == null || name.isBlank()) {
      throw new HolmgangException("an object template must have a name");
    }
    if (kind == Kind.DEPLOYMENT) {
      if (module.isEmpty()) {
        throw new HolmgangException("a deployment object must name a module: " + name);
      }
      if (replicas < 1) {
        throw new HolmgangException("a deployment object must have >= 1 replicas: " + name);
      }
    }
  }
