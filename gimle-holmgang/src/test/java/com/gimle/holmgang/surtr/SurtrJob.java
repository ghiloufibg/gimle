...
public record SurtrJob(
  ...
  public enum Type {
    ...
    static Type fromField(final String field) {
      return switch (field) {
        ...
        case "churn" -> CHURN;
        case "delete" -> DELETE;
        default -> throw new HolmgangException("unknown job type: " + field);
      };
    }
  }
  ...
  public enum ChurnMode {
    ...
    static ChurnMode fromField(final String field) {
      return switch (field) {
        ...
        default -> throw new HolmgangException("unknown churn mode: " + field);
      };
    }
  }
  ...
  public SurtrJob {
    if (name == null || name.isBlank()) {
      throw new HolmgangException("a job must have a name");
    }
    if (type == null) {
      throw new HolmgangException("a job must have a type: " + name);
    }
    objects = List.copyOf(objects);
    switch (type) {
      case CREATE -> {
        if (iterations < 1) {
          throw new HolmgangException("a create job must have >= 1 iterations: " + name);
        }
        if (objects.isEmpty()) {
          throw new HolmgangException("a create job must declare at least one object: " + name);
        }
        if (qps <= 0) {
          throw new HolmgangException("a create job must have a positive qps: " + name);
        }
        if (burst < 1) {
          throw new HolmgangException("a create job must have burst >= 1: " + name);
        }
      }
      case CHURN -> {
        if (churn == null) {
          throw new HolmgangException("a churn job must declare a churn section: " + name);
        }
        if (churn.percent() < 1 || churn.percent() > 100) {
          throw new HolmgangException("churn.percent must be 1..100: " + name);
        }
        if (target.isEmpty()) {
          throw new HolmgangException("a churn job must name a target create job: " + name);
        }
      }
      case DELETE -> {
        if (target.isEmpty()) {
          throw new HolmgangException("a delete job must name a target create job: " + name);
        }
      }
    }
  }
