...
public record Pool(FaultKind kind, int weight, Duration dwell) {
  ...
  public Pool {
    if (kind == null) {
      throw new HolmgangException("a pool must name a fault kind");
    }
    if (weight < 1) {
      throw new HolmgangException("pool weight must be >= 1, got " + weight + " for " + kind);
    }
    if (dwell == null || dwell.isNegative()) {
      throw new HolmgangException("pool dwell must be a non-negative duration for " + kind);
    }
  }
