...
Feature: Deployment lifecycle
  ...
  Scenario: State written through one control-plane replica serves through another
    ...
    Then within 120s deployment "fabric-provider" is ACTIVE observed via control-plane replica 1
    And within 120s deployment "fabric-consumer" is ACTIVE observed via control-plane replica 1
    And within 60s instance 0 of "fabric-consumer" logs "Hello, Gimlé!"
