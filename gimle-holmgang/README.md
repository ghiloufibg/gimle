...
root's `RTM.md` (generated by cross-referencing `REQUIREMENTS_MATRIX.md` against this module's own
Cucumber suite) and fails the whole run if any *implemented* requirement (anything not marked
`Removed`) still reports `Coverage: Not Covered` -- a release-readiness gate, not a scenario. It
runs last, after the file-writing/reporting plugins above, and never interrupts or skips a scenario
that's already running: it only ever adds one final pass/fail check once the suite is otherwise
done. On success it prints a one-line summary (`RTM check passed: 42/42 requirements covered by
Holmgang Cucumber tests.`); on failure it prints every uncovered requirement's ID, feature name,
and category, followed by what to do about it.
...
| `-Dgimle.holmgang.rtmCheck.file` | `../RTM.md` | Override where `RTM.md` is read from, relative to this module's own working directory (the repo root by default). |
...
If the gate is enabled and `RTM.md` is missing or unreadable, the run fails loudly explaining
where it looked and how to fix it (generate the file, point `-Dgimle.holmgang.rtmCheck.file` at
the right one, or disable the gate) -- it never silently skips the check just because the file
wasn't found. Parsing itself only reads `RTM.md`'s own machine-generated `## Summary Table` (one
row per requirement: ID, Feature, Status, Coverage) plus its `## Coverage Gaps` table for each
uncovered requirement's Category; it tolerates reordered or extra columns in either table, but
fails loudly rather than silently passing if the summary table itself can't be found or parses to
zero rows.
