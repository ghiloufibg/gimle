...
if defined JAVA_HOME (
  ...
  if not exist "!java_bin!" (
    echo gimle: JAVA_HOME is set to "%JAVA_HOME%" but !java_bin! does not exist -- fix or unset JAVA_HOME 1>&2
    exit /b 1
  )
  ...
  if !java_major! lss 25 (
    echo gimle: JAVA_HOME is set to "%JAVA_HOME%", but !java_bin! reports "!version_line!" -- Gimle requires JDK 25+; point JAVA_HOME at a JDK 25+ install or unset it 1>&2
    exit /b 1
  )
) else if exist "%script_dir%..\jre\cli" (
  ...
  if not exist "!java_bin!" (
    ...
    echo gimle: this archive's bundled JRE ^(built for !bundled_platform!^) does not run on this machine -- ignoring it and looking for a java of this machine's own 1>&2
...
if "%java_bin%"=="java" (
  echo gimle: no Java runtime to run on -- put a JDK 25+ java.exe on PATH, or set JAVA_HOME to a JDK 25+ install 1>&2
  exit /b 1
)
