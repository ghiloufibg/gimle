...
if defined JAVA_HOME (
...
) else if exist "%script_dir%..\jre\hilmir" (
  ...
  if not exist "!java_bin!" (
    ...
    echo hilmir: this archive's bundled JRE ^(built for !bundled_platform!^) does not run on this machine -- ignoring it and looking for a java of this machine's own 1>&2
...
if "%java_bin%"=="java" (
  echo hilmir: no Java runtime to run on -- put a JDK 25+ java.exe on PATH, or set JAVA_HOME to a JDK 25+ install 1>&2
  exit /b 1
)
