...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 952 | The whole denominator before any classification. |
...
| **User-observable** | **679** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 655 | **96.5%** of the user-observable set — meets the 90% target. |
...
| **DEV** | Dev — Module author | A developer writing their first module against the docs, using the Maven plugin and hilmir tooling from a source checkout — never a running production cluster. | G A | 7 | 97 |
