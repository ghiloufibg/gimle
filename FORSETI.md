...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 927 | The whole denominator before any classification. |
...
| **User-observable** | **654** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 630 | **96.3%** of the user-observable set — meets the 90% target. |
...
| **DEV** | Dev — Module author | A developer writing their first module against the docs, using the Maven plugin and hilmir tooling from a source checkout — never a running production cluster. | G A | 7 | 72 |
