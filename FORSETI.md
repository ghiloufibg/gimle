...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 843 | The whole denominator before any classification. |
...
| **User-observable** | **575** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 552 | **96.0%** of the user-observable set — meets the 90% target. |
...
| **NET** | Fabric & Networking | Whoever wires two teams' services together: Services, DNS, the node proxy, the gateway, network policy. | A B C | 10 | 70 |
...
| **ART** | Artifacts & Registry | A release engineer publishing module builds and expecting the registry to behave like Nexus. | A B | 8 | 28 |
| **OBS** | Observability & Console | An on-call engineer with nothing but the consoles and `gimle logs`, at 3 a.m. | A B | 10 | 79 |
