...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 778 | The whole denominator before any classification. |
...
| **User-observable** | **517** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 517 | **100.0%** of the user-observable set — meets the 90% target. |
...
| **DEP** | App-1 — Deployments | A developer shipping a long-running service and living with it: redeploys, rollbacks, probes, tiers. | A | 13 | 55 |
...
| **NET** | Fabric & Networking | Whoever wires two teams' services together: Services, DNS, the node proxy, the gateway, network policy. | A B C | 10 | 58 |
