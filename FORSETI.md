...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 845 | The whole denominator before any classification. |
| Out of scope | 69 | Not built, a documented limitation, or a test asset itself — see the exclusions table. |
| Internal | 200 | Real platform behaviour a user cannot observe from outside a process; every row carries its unit-test or Holmgang citation (Holmgang 26, unit 174, uncited 0). |
| **User-observable** | **576** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 552 | **95.8%** of the user-observable set — meets the 90% target. |
...
|---|---|---|---|---:|---:|
| **OPS** | Ops — Platform operator | The person who unpacks the archives and stands the cluster up, keeps it up, upgrades it, and restores it after a bad day. | A B C | 15 | 66 |
