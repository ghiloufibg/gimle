...
| **NET** | Fabric & Networking | Whoever wires two teams' services together: Services, DNS, the node proxy, the gateway, network policy. | A B C | 10 | 64 |
...
| GIMLE-797 | `gimle-gateway` | Gateway per-host TLS certificate bindings (gateway.tlsCertificates) reload on a config change without a restart | observable | FLEET | NET-5 |
<!-- /forseti:generated -->
