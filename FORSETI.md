...
| **BATCH-7** | A | `apply -f` an ArtifactSet bundling several workload kinds under per-module tenant tags, with one member deliberately broken. | Healthy members come up together; the failure is reported against that member by name, not the whole bundle opaquely. | GIMLE-577 |
...
| GIMLE-788 | `gimle-cli` | CliExtension seam dispatches an unrecognized verb to a ServiceLoader-discovered provider | internal | UNIT | gimle-cli's CliExtensionSeamTest (classpath discovery via a test-only provider, dispatch, help folding, unknown-verb error preserved) and gimle-hugin's HuginExtensionTest. A further CliExtensionSeamTest case pins the scoped `-h` output. |
| GIMLE-789 | `gimle-cli` | An extension is handed a read-only view of the control-plane API, never the client | observable | FLEET | BATCH-3 |
| GIMLE-790 | `gimle-hugin` | `gimle top` renders a live, read-only cluster view of nodes and instances | observable | FLEET | OBS-9 |
| GIMLE-791 | `gimle-hugin` | A failed poll keeps the last good rows and ages them rather than clearing the screen | observable | FLEET | NET-6 |
| GIMLE-792 | `gimle-hugin` | Instance drill-down with lifecycle timeline and a live log tail | internal | UNIT | gimle-hugin's InstanceWatcherTest (backlog-then-follow ordering, the resume cursor, tenant scoping on every route, a failing route, a stream ending on its own) and InstanceScreenTest, plus SnapshotReaderTest's tier/limit parsing cases and InstanceScreenTest's per-tier rendering cases. |
