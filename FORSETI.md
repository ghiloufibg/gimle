...
| ID | Env | Objective | Oracle | Requirements |
|---|---|---|---|---|
| **DEP-1** | A | Write a Deployment manifest from scratch (no copy-paste of a bundled example), `apply -f` it, watch it reach ACTIVE via `get deployments`, `get instances`, `gimle events`, and the console's Deployments/Instances/detail pages. | Every lifecycle transition (INSTALLED→RESOLVED→STARTING→ACTIVE) is a durable event visible in the CLI timeline and the instance detail page; table and JSON output agree. The Applications screen reads the same deployment as Healthy/Synced once it is placed, and names the reason whenever it does not. | GIMLE-371, GIMLE-274, GIMLE-219, GIMLE-045, GIMLE-084, GIMLE-032, GIMLE-377, GIMLE-437, GIMLE-439, GIMLE-444, GIMLE-753, GIMLE-157, GIMLE-779 |
...
| GIMLE-779 | `gimle-console` | An Applications addon presenting every deployable resource as one application, with health and sync as separate verdicts and a resource tree beneath each | observable | FLEET | DEP-1, BATCH-1, OBS-7, JRN-7 |
<!-- /forseti:generated -->
