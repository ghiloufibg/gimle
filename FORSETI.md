...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 916 | The whole denominator before any classification. |
...
| **User-observable** | **643** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 619 | **96.3%** of the user-observable set — meets the 90% target. |
...
| **DEV** | Dev — Module author | A developer writing their first module against the docs, using the Maven plugin and hilmir tooling from a source checkout — never a running production cluster. | G A | 7 | 62 |
...
| **NET** | Fabric & Networking | Whoever wires two teams' services together: Services, DNS, the node proxy, the gateway, network policy. | A B C | 10 | 76 |
