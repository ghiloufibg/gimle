...
|---|---:|---|
| Requirements in `requirements-matrix.json` | 968 | The whole denominator before any classification. |
| Out of scope | 70 | Not built, a documented limitation, or a test asset itself — see the exclusions table. |
| Internal | 205 | Real platform behaviour a user cannot observe from outside a process; every row carries its unit-test or Holmgang citation (Holmgang 26, unit 179, uncited 0). |
| **User-observable** | **693** | The capability set the fleet is measured against. |
| Reached by a fleet scenario | 667 | **96.2%** of the user-observable set — meets the 90% target. |
...
| **DEV** | Dev — Module author | A developer writing their first module against the docs, using the Maven plugin and hilmir tooling from a source checkout — never a running production cluster. | G A | 7 | 105 |
...
| **CHAOS** | Exploratory, Chaos & Negative-path | Mildly adversarial, reads no manual twice, and runs the shipped chaos tooling against the cluster. | A B C | 10 | 56 |
...
|---|---|---|---|---|
| **OPS-1** | G | Unpack every distribution archive (`gimle-platform`, `gimle-cli`, `gimle-hilmir`, `gimle-midgard`, `gimle-ragnarok`, `gimle-ivaldi`); verify each `.sha256` against its archive; open each archive's CycloneDX SBOM; run each `bin/` launcher with `-h`. | Checksums pass; each SBOM lists that archive's own dependency set, not a copy of another's; every launcher prints scoped help and exits 0 -- `hilmir -h` and `ivaldi -h` both included, not treated as an unknown token: IvaldiMain parses real program arguments of its own (`--port`/`--data-root`/`--host`, `-h`/`--help` among them), and the wrapper script routes `-h` after `com.gimle.ivaldi.IvaldiMain` on the `java` command line rather than before it, so it reaches IvaldiMain as a program argument and prints its own scoped USAGE text, not the underlying `java` launcher's generic one. | GIMLE-560, GIMLE-561, GIMLE-562, GIMLE-564, GIMLE-611, GIMLE-635, GIMLE-642, GIMLE-849, GIMLE-909 |
...
| **CHAOS-4** | A | No `--server` and no `GIMLE_SERVER`; a manifest path that does not exist; `-h` at every verb level; a wrong flag; two positional names to a single-resource verb; switch clusters with `gimle context`. Then point `--server` at a listener that accepts the connection and never answers, and run a mutating verb. | Every failure exits with a code that names why, shows usage where a flag was wrong, rejects the extra positional rather than truncating, and gives a message a first-time user can act on; contexts switch cleanly. The hung write exits 6 after the 10s timeout and says the outcome is *unknown* and may already have been applied, quoting the `X-Gimle-Request-Id` a retry can present -- never that the server could not be reached. (The half that cannot be staged here is a write that genuinely commits server-side while the client's answer is lost; the message and exit code are what this scenario reaches.) | GIMLE-653, GIMLE-761, GIMLE-665, GIMLE-765, GIMLE-635, GIMLE-894, GIMLE-966, GIMLE-968 |
| **CHAOS-5** | B | Fire a scale-up and a delete at the same deployment back to back; then two concurrent applies from two control-plane replicas. Then, with `curl`, send the same `POST /deployments/{name}/rollback` twice under one `X-Gimle-Request-Id` (the second time deliberately at the other replica); repeat with the same id on `PUT /deployments/{name}` after a different apply has landed in between; and send a keyed rollback with an id of six characters and one containing a space. | Converges to deleted with no half-scaled zombie; the concurrent applies are generation-guarded so one loses with a conflict rather than a silent lost update. The repeated rollback mints exactly one revision, and the second response carries `X-Gimle-Replayed: true` with the first response's body byte for byte -- including when it is answered by the replica that never saw the original, since the receipt is replicated state rather than one replica's memory. The replayed `PUT` returns its own original 200 and the newer apply's content is still what is deployed, not the retry's. A malformed id is refused 400 naming the header, with no revision minted. The same rollback sent twice with no id at all still mints two revisions. | GIMLE-646, GIMLE-241, GIMLE-874, GIMLE-963 |
...
| **CHAOS-8** | C | As a tenant-B account, attempt every action GOV-3 confirmed tenant-A-only, straight at the API rather than through the console; also directly at Fafnir, Andvari and Muninn. Then take an `X-Gimle-Request-Id` that tenant A used on a keyed write and re-present it on B's own rollback of a resource B may write. | Refused identically (403) everywhere, independent of any UI-level hiding. B's request under A's id is not replayed: it carries no `X-Gimle-Replayed` header, executes as B's own write, and its response reveals nothing about the id already existing -- while A's own retry under that id still replays A's original outcome afterwards. (Reachable only here: on a plaintext listener every caller is the same anonymous principal, so receipts are not separated at all.) | GIMLE-250, GIMLE-285, GIMLE-310, GIMLE-691, GIMLE-964 |
...
| `raft-internals` | Raft safety/liveness mechanics below the level a user observes (they observe 'the cluster kept accepting writes', which the fleet does test). | 25 | Holmgang 15, unit 10 |
...
| `process-plumbing` | Control-channel bootstrap, tick loops, executor isolation, stdout draining, lease bookkeeping, ledger persistence. Mechanism, not a user-facing capability. | 43 | Holmgang 2, unit 41 |
...
|---|---|---|---|---|---|
| 0.1.0-alpha.2 | 2026-09-02/03 | Forge (G) + Midgard (A) + Fleet (B) + Vault (C) — all four built | Waves 0–3, full doctrine scope plus a 3 Sep retry/mop-up round that closed every scenario an earlier round left not-executed or blocked by instability (SCHED-2/3/9/10, NET-2/3/7/9 on Fleet; GOV-9's volume-destroy half, NET-6, NET-10's byte-forwarding leg on Vault). Two named remainders survive: OPS-9 (time budget, never attempted) and the console CA-trust gap across GOV-3/6/SEC-3/7 (now fully diagnosed, exact fix flags identified for a future run). Percentage not computed against the full requirement denominator this run | 7 / 66 / 40 / 4 | [Forseti Verdict](https://claude.ai/code/artifact/3a5009c5-df2b-4401-b7bc-a08aeeb30bc7) |
| 0.1.0-alpha.2 | 2026-09-05 | Forge (G) + Midgard (A) + Fleet (B) + Vault (C) — all four built fresh at commit f9ebc3c8 (142 commits after the first pass) | Wave 0 (bring-up) and Wave 1 (baseline, all three environments in parallel) executed in full; Wave 2 (pressure/chaos — OPS-6/7/9/10/11, SCHED-4/5/6, CHAOS-1…10) not run this pass, closed out after sustained host-level Docker exhaustion from running all three multi-container environments concurrently under 20+ agents (see the artifact's own Method section). Several Fleet/Vault Wave-1 scenarios landed partial rather than full evidence for the same reason (SEC-5…12 and NET-6/10 on Vault; SCHED-1/2/7/8/9/10, OBS-3…6, BATCH-3/5 and ART-8 on Fleet) — recorded as not-executed/partial, never as passed. Two run-scaffolding defects (a Fleet compose recreate-cascade bug; the shared entrypoint's all-or-nothing machine supervision) were root-caused and excluded from the platform tally. First-pass blocker B3 (`secret retire-key`) was incidentally re-tested and found still open; B1/B2/B4/B5/B6/B7 were not re-verified this run | 8 / 31 / 22 / 3 | [Forseti Verdict II](https://claude.ai/code/artifact/9a45f7db-32c3-4e2a-b254-dbc4b4502144) |
| 0.1.0-alpha.2 | 2026-09-07 | Forge (G) + Midgard (A) + Fleet (B, 5→6→4-store-replica) + Vault (C) — all four built fresh at commit faf7bd73, sequentially (Forge+Midgard, then Fleet, then Vault, each stopped before the next began, for both stages of the run) to avoid the second pass's own host-exhaustion pattern | Full doctrine scope, run in two stages. Wave 0 (bring-up) and Wave 1 (baseline) ran first, landing clean with full evidence on all four environments; Wave 2 (pressure/chaos) followed on the same run, restarting each environment from its own preserved Wave-1 state rather than rebuilding. Fleet's store/control-plane-critical Wave-2 scenarios (a membership change, a leader kill, a rolling upgrade) were deliberately staggered one at a time rather than run concurrently, given the store's already-known Raft fragility; every other Wave-2 batch ran fully concurrently. 34 agents total, 91 findings after Lead dedup (13 raw findings merged into 6). First-pass blocker B3 (`secret retire-key`) confirmed open a third time with its exact reachability gap isolated (the guard only checks the currently-active key); B4 (revoked certificate accepted) is now fixed for the control plane/Fafnir/Andvari but still open at Muninn; B5 (Bifrost byte-forwarding under mTLS) is confirmed genuinely fixed. Four new blockers: the `gimle-system` reserved-tenant veto has no effect on CONFIG/SECRET/LimitRange writes nor on DELETE for DaemonSet/Service; a client certificate silently overrides a console session cookie's identity for both authorization and audit under mTLS; a node agent that OOMs becomes a zombie process the documented recovery command mistakes for healthy, indefinitely. The store's Raft leader-election fragility (first surfaced at bring-up in this run's own Wave 0) was confirmed under two further independent Wave-2 triggers (a full control-plane-tier restart; a store peer temporarily out of sync with its own membership), filed Major throughout since a full recovery exists and was verified every time | 6 / 45 / 38 / 2 | [Forseti Verdict III](https://claude.ai/code/artifact/fcf9e2bd-9155-47d3-85d2-5a2329bcdcab) |
...
| GIMLE-962 | `gimle-mimir` | Request-outcome receipts are replicated store state, recorded once per request id and expired by an explicit cutoff | internal | UNIT | `RequestOutcomeStoreTest` (five tests against a real `StateStore`: snapshot/restore round trip, sweep removes expired and keeps fresh, a receipt exactly at the cutoff is kept, first-recorded-wins, unknown id absent); `RaftCodecTest` (a receipt-bearing `StateSnapshot` survives the wire round trip); `RaftLogTest`/`RaftNodeSafetyMechanicsTest` updated for the new snapshot component. |
| GIMLE-963 | `gimle-controlplane` | A repeated mutating request carrying the same `X-Gimle-Request-Id` is answered from the first attempt's recorded outcome instead of running again | observable | FLEET | CHAOS-5 |
| GIMLE-964 | `gimle-controlplane` | A recorded request outcome is replayed only to the principal that recorded it, and a mismatch reads as a miss rather than a refusal | observable | FLEET | CHAOS-8 |
| GIMLE-965 | `gimle-controlplane` | Request-outcome receipts are swept off the reconcile tick after a fifteen-minute retention window | internal | UNIT | `RequestOutcomeSweeperTest` (three tests against a real `StateStore` and a fixed `Clock`: expired removed and fresh kept; a tick with nothing expired proposes no mutation at all, asserted against the recorded proposal list; a single tick converges a long-untouched table). `RequestOutcomeStoreTest` pins the strictly-before cutoff comparison. |
| GIMLE-966 | `gimle-cli` | Every mutating CLI request identifies itself, and a write whose answer is lost is reported as an unknown outcome rather than a failure | observable | FLEET | CHAOS-4 |
| GIMLE-967 | `gimle-ivaldi` | The Runner page shows a CronJob's own firing history, not just its initial deploy status | observable | FLEET | DEV-7 |
| GIMLE-968 | `gimle-cli` | Built-in resource nouns accept both singular and plural spellings consistently across get/set/delete | observable | FLEET | CHAOS-4 |
<!-- /forseti:generated -->
