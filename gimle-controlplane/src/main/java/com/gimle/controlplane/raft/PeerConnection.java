 ...
 * SocketChannel} to a single peer (design §2.2) -- not {@code FabricClient}'s open-per-call
 * pattern, because Raft contacts a fixed, small peer set every 50ms indefinitely for the node's
 * entire lifetime as leader/candidate, so a fresh handshake per RPC is needless churn a fabric-
 * style occasional cross-hop call doesn't have to accept. Any {@link IOException} closes the
 * channel and lets the *next* call attempt reopen it -- failures are not retried within one call,
 * matching the "acceptable, bounded gap" tolerance already established elsewhere in this design
 * (heartbeat repopulation after a leader change) rather than inventing a retry-with-backoff policy
 * nothing asked for.
 */
