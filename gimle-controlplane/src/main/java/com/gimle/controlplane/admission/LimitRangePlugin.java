 ...
 * cross-deployment summation to compute. An unenforceable {@code tenantId} (see {@link
 * Tenant#isEnforceable}) or an absent LimitRange for the tenant are both a no-op allow -- a
 * LimitRange is opt-in per tenant, not a default every deployment must satisfy. The bound check
 * itself is {@link LimitRangeSpec#violation}, shared with {@code LimitRangeReconciler} so the two
 * never drift on what counts as a violation.
 */
