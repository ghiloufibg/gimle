 ...
 * last-writer-wins by {@code (node, workerId, moduleId)} within one {@code export}'s bucket.
 * Primarily {@link ServiceCatalog}'s own gossip-piggyback wire shape, but public (and observable
 * via {@link ServiceCatalog#onDelta}) because a node agent needs this exact shape to relay a
 * newly-learned delta down to its own supervised workers over the control channel.
 */
