 ...
 * directly. Every inbound HTTP request to {@code /call} on {@link #PORT} does exactly one real,
 * synchronous {@code lookupService(Greeter.class)} + {@code greet(...)} call and reflects the
 * outcome in the HTTP response -- so the request rate an external tool controls becomes
 * greeter-provider's own real, worker-reported {@code requestRatePerSecond}, not a synthetic
 * stand-in for it. {@link #PORT} is a fixed constant, not delivered config: this module exists
 * solely for {@code gimle-smoke-tests}' own fixed-topology cluster, the same reasoning every other
 * port in that suite is a hardcoded constant.
 *
