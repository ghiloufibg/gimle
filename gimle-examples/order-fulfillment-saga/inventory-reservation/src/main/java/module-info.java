 ...
 * these types).
 */
module com.example.orderfulfillment.inventory {
  requires static com.gimle.module;
  requires static org.slf4j;

  // FabricServer (gimle-fabric) reflectively invokes InventoryReservationService methods on the
  // registered instance from outside this module -- a public type alone isn't enough for that
  // under JPMS strong encapsulation, its package must be exported too.
  exports com.example.orderfulfillment;
  // InventoryReservationHooks: ModuleController instantiates this reflectively from outside the
  // module, the same way it does with every long-running module's own ModuleLifecycleHooks.
  exports com.example.orderfulfillment.inventory;
}
