...
export class HttpNetworkPoliciesRepository implements NetworkPoliciesRepository {
  ...
  async fetchOne(name: string, tenantId: string): Promise<NetworkPolicy> {
    return requestJson<NetworkPolicy>(
      "GET",
      `/networkpolicies/${encodeURIComponent(name)}${tenantQuery(tenantId)}`,
    );
  }
  ...
  async remove(name: string, tenantId: string): Promise<void> {
    await requestOk(
      "DELETE",
      `/networkpolicies/${encodeURIComponent(name)}${tenantQuery(tenantId)}`,
    );
