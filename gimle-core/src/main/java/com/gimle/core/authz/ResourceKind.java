 ...
 * {@code SECRET} guards a tenant's {@code ConfigEntry}s written with {@code encrypted=true},
 * distinct from {@code CONFIG} (plaintext entries) so a role can be granted one without the other
 * -- {@code BuiltinRoles.CLUSTER_ADMIN} picks it up automatically since it iterates {@link
 * #values()} at class-load time.
 */
