 ...
 * mTLS client certificates; this exists specifically because a browser session (see {@code
 * claudedocs/authn-authz-design.md} §6) has no certificate to prove itself with. {@code
 * passwordHash} follows {@code SecretCipher}'s existing "self-contained {@code byte[]}" convention
 * (defensively cloned in and out, matching {@link com.gimle.core.config.ConfigEntry#value()}).
 */
