...
/**
 * A module's declared need for local-disk-backed persistent storage (priority-3 design doc §5),
 * sibling to {@link ResourceSpec}: a {@code StatefulSet}-shaped module declares {@code volume:} in
 * its own {@code gimle-module.yaml}, not in the workload manifest -- the same place {@code
 * isolation:}/{@code resources:} already live, since this is a property of the artifact itself, not
 * of how many replicas an operator asks for. {@code sizeBytes} is advisory, checked once at {@link
