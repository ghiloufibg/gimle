...
Phases 1–5 are implemented and committed: `gimle-core`, `gimle-module`, `gimle-os`, `gimle-worker`, `gimle-agent`, `gimle-controlplane`, `gimle-observability`, `gimle-fabric`, and `gimle-cli` all exist as working Maven modules with tests. `gimle-api` doesn't exist as its own module — a deliberate deviation, not a gap; probe/service-registry types live in `gimle-module` instead. Phase 6 (module web console) is stretch-goal work: the control-plane-side static-serving backend (`ApiServer.serveConsole`, `ControlPlaneMain`'s `--console-dir` flag) has landed, and the frontend at `gimle-console/` — a Lovable-generated React/TanStack Router/Zustand/Tailwind SPA — now talks to the real control-plane API via `Http*Repository` implementations (composition root swapped, dev proxy wired; see `claudedocs/web-console-design.md` §10 for how) rather than the original mock-only pull. Screens: Overview, Metrics, Topology, Deployments, Instances, Nodes, Tenants, Config (see `web-console-design.md` §4/§4c) plus a display/density/theme preference panel. Test coverage (Vitest, `web-console-design.md` §12/M4) is still partial. `gimle-console/` is a plain Vite SPA build (`bun run build` → `dist/index.html` + `dist/assets/*`), not a Maven module and not TanStack Start/SSR — matches the same pattern already proven in the sibling `flow-trace-ui` project. Build/test with `mvn verify` from the repo root (requires JDK 25 on `PATH`/`JAVA_HOME`); `gimle-console/` is built independently with Bun, not part of the Maven reactor. Logs are now backed by a real `/logs/*` API (`gimle-console/src/repositories/http/logs.ts`, `AgentLogServer` in `gimle-agent`, proxied through `ApiServer` in `gimle-controlplane`), not a mock repository — see `claudedocs/log-explorer-design.md` for the design and `gimle-console/LOCAL_DEV.md` for how to run a real control-plane + node-agent + deployed-module cluster locally and exercise it end to end (including live log tailing from both the console UI and `gimle-cli logs --follow`).
