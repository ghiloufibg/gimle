...
`gimle-smoke-tests` (opt-in via `-Psmoke`, not part of default `mvn verify`) is the automated version of "deploy to a real server": `GreeterSmokeTestIT` spawns a genuine `ControlPlaneMain` + `AgentMain` (+ its own `WorkerMain` child) subprocess cluster on ports distinct from `LOCAL_DEV.md`'s manual walkthrough, plus two `FafnirMain` replicas sharing one secret key file, deploys both greeter modules through the real HTTP API — the provider tenant-scoped, with a secret written through the real `/secrets/*` API before deployment — asserts both reach `ACTIVE`, that the consumer's real fabric call actually shows up in its own log, and that the provider's own log shows the secret value it fetched back via `ctx.config(...)`, then runs the console's Playwright suite against that same live cluster — one command (`mvn -pl gimle-smoke-tests verify -Psmoke`) exercises the whole stack, real processes and a real browser, start to finish.
...
Five Java process roles, nothing else runs on the machine:
...
- `gimle-smoke-tests` — test-only module, no main sources: `GreeterSmokeTestIT` spins up a real subprocess cluster, deploys the greeter modules through the real API, and runs the console's Playwright suite against it; opt-in via `-Psmoke`, not part of default `mvn verify`
...
- **No `claudedocs/` references from committed files.** Comments, Javadoc, and config files under version control (`pom.xml`, `.mvn/**`, `*.java`, `*.xml`, `*.yaml`) must never point a reader at a `claudedocs/*.md` path. `claudedocs/` is gitignored and local to one checkout — a reference to it from a committed file is a dangling link for anyone else who clones the repo, or once that local doc is deleted. State the *why* inline in the comment itself instead of citing an external doc for it.
