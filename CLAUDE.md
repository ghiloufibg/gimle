...
`gimle-smoke-tests` (opt-in via `-Psmoke`, not part of default `mvn verify`) is the automated version of "deploy to a real server": `GreeterSmokeTestIT` spawns a genuine `ControlPlaneMain` + `AgentMain` (+ its own `WorkerMain` child) subprocess cluster on ports distinct from `LOCAL_DEV.md`'s manual walkthrough, deploys both greeter modules through the real HTTP API, asserts both reach `ACTIVE` and that the consumer's real fabric call actually shows up in its own log, then runs the console's Playwright suite against that same live cluster — one command (`mvn -pl gimle-smoke-tests verify -Psmoke`) exercises the whole stack, real processes and a real browser, start to finish.
