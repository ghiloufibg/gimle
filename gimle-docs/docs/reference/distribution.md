...
|---|---|---|
| `gimle-cli-<version>-cyclonedx.json` | `gimle-hugin`'s own `cyclonedx-maven-plugin` execution | `gimle-hugin`'s runtime dependency closure — `gimle-cli`'s whole closure plus `jline-terminal`/`jline-terminal-ffm` — which is exactly what that archive ships. |
| `gimle-hilmir-<version>-cyclonedx.json` | `gimle-hilmir`'s own execution | `gimle-hilmir`'s runtime dependency closure — exactly what that archive ships. |
...
the whole dependency graph of whichever Maven project it runs in. `gimle-hilmir` and `gimle-ragnarok`
are each already the graph their own archive ships, so their SBOMs come straight from their own
execution. The CLI archive is the exception: `gimle-cli` itself never depends on
`jline-terminal`/`jline-terminal-ffm` or on `gimle-hugin` — `gimle-hugin` implements the
`CliExtension` `gimle-cli` defines, so the dependency runs the other way, and `gimle-cli` depending
back on `gimle-hugin` would be a circular reactor dependency Maven rejects — so `gimle-cli`'s own
graph structurally cannot describe the CLI archive's real jar set. `gimle-hugin`'s graph is the one
that does, so its own `makeBom` execution backs the CLI archive's SBOM instead. Only the two graphs
`gimle-dist` genuinely describes (platform, midgard) are generated in `gimle-dist` itself.

`makeBom` also always writes the running Maven project as the SBOM's root `metadata.component`,
with no way to override it, so the platform and midgard files would arrive naming `gimle-dist` — a
module that ships nothing an operator can install — and the CLI file would arrive naming
`gimle-hugin` rather than the `gimle-cli` archive it sits beside. `gimle-dist`'s
`postprocess-archives` step rewrites that one root component (name, description, and the coordinate
its `bom-ref`, `purl`, and dependency-graph root ref all share) on all three copies to name the
archive the file actually describes. Nothing below the root is touched: the resolved dependency
components, which are what an SBOM is read for, come straight from the generator. A build where a
rewrite stops matching fails rather than shipping a mislabelled SBOM.

## What v1 deliberately leaves out

- **No git-tag-driven versioning.** Every module in this repo builds as `0.1.0-beta.1` today, a
