...
| `files` | no | A list of `{path, config}` or `{path, secret}` entries — renders that key's tenant-scoped value, verbatim (no templating), to `path` before the process starts. `path` is relative to the instance's own per-instance data root, or absolute. A `secret:`-backed entry (the Kubernetes secret-volume-mount analogue) is fetched from Fafnir over the agent's own mTLS node identity and written with owner-only file permissions, via the portable `File.setReadable`/`setWritable` calls. |
