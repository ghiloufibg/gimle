...
| `gimle-ivaldi` | The cluster designer's backend (`IvaldiMain`) — not a cluster process kind: stores Blueprint documents as flat JSON files and validates their rendered `topology.yaml`/`bundle.yaml`/manifest output against the real `gimle-hilmir`/`gimle-mimir` parsers and validators (`TopologyParser`/`TopologyValidator`/`BundleParser`/`ManifestParser`) over a small JSON HTTP API. Deliberately unauthenticated and loopback-bound by default, the same posture `gimle-saga` already established, but distributed like an operator/developer tool rather than kept dev-tree-only like Saga: `mvn gimle:ivaldi`/`gimle:ivaldi-stop` cover the source-checkout workflow, and a standalone `gimle-ivaldi-<version>.tar.gz` archive (also bundled into the platform archive) covers running it with no checkout at all — see [Distribution archives](../reference/distribution.md). Serves the bundled `gimle-ivaldi-console` SPA at `/console`. Saved cluster connections (`/api/clusters[/{id}[/topology]]`, flat JSON via `ClusterStore`) and actually running a Blueprint against one (`/api/runs*` via `RunController`) are both implemented: every step — `MachineLauncher.up`/`down` for the platform process tree, jar-sourced artifact pushes, and `BundleRenderer`/`ReleaseReconciler` for the application deploy — runs in-process against `gimle-hilmir`'s own release/launch classes, no `hilmir` subprocess. A topology naming more than one machine boots each machine's own processes concurrently on its own virtual thread (`RunController.upAllMachines`), the same real per-machine loopback-IP simulation (`127.0.0.2`, `127.0.0.3`, ...) `gimle-hilmir` itself already supports rather than Docker or any other container runtime, with any cross-machine boot-order dependency resolved through `MachineLauncher`'s own remote-readiness wait rather than a fixed start order. `RunController` diffs a run's rendered `topology.yaml` against the text it last actually applied to that cluster (kept by `ClusterStore` alongside the connection) to decide deploy-only vs. reboot. A cluster's infra is not owned by any one blueprint: once its topology is up, any number of blueprints may each deploy their own bundle onto it as long as they render the same topology, each tracked as its own independently-run, -stopped and -logged deployment (`runsByDeployment`, keyed by cluster and blueprint together); a topology *change* is refused outright while another blueprint's own deployment still shares that cluster, and stopping one deployment on a shared cluster only undeploys its own release, tearing the shared infra down only once it is the last one left. Not built yet: the tier-3 `POST /api/runs/current/dry-run` proxy, and a per-cluster TLS identity for this controller's own outbound calls (today it follows `IvaldiMain`'s own process-wide transport config, like every other Gimlé tool). |
| `gimle-ivaldi-console` | Ivaldi's own web console SPA — the same no-Java Bun/Vite pattern as the other consoles, embedded into `gimle-ivaldi`'s jar and served from there: a drag-and-drop canvas (`@xyflow/react`) for laying out a Blueprint's platform topology and application, an Inspector, and Problems/Files drawers, with tier-1 validation running client-side and tier-2 validation against the real backend's `POST /api/validate`. Saved clusters and the Run drawer are wired to the real same-origin `/api/clusters*`/`/api/runs*` backend above — no mock or `localStorage` path remains in the composition root; since that backend reports a coarse status plus a plain-text log rather than an event stream, the console polls it and derives its per-step run timeline from the exact log lines `RunController` writes (`src/lib/runPhases.ts`). The Runner page's own Machines section groups a multi-machine topology's roles by the machine each is placed on, derived client-side from the same rendered `topology.yaml` the run itself was started from (or, once attached to an already-running deployment, from the cluster's last-applied topology) rather than from any backend process listing. |
