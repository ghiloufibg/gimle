...
public final class ProbeLoop implements AutoCloseable {
  ...
  /**
   * {@code initialDelay} (P2-4) is the delay before the *first* tick, independent of {@code
   * interval} -- lets a module declare its own post-{@code onStart} warmup window (lazy init, cache
   * fill, JIT) without shrinking every subsequent tick's spacing to match. A module with no
   * declared {@code initialDelaySeconds} gets the pre-P2-4 behavior via the shorter overload above,
   * which passes {@code interval} here unchanged.
   */
  public void start(
      String key,
      BoundedModuleScheduler moduleScheduler,
