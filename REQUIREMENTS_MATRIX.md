...
- **Status**: Complete. StateMutation.Batch carries N non-nested mutations in one log entry, applied in order; MutationSink.proposeAll wraps a burst (empty list is a no-op, a single mutation is proposed bare) and rides the existing single-mutation Propose RPC unchanged, so RaftNode and StoreClient both inherit it with no wiring change. DeploymentReconciler, StatefulSetReconciler, and DaemonSetReconciler batch their burst sites -- stale-workload sweeps, scale-down reclaims/evictions, multi-replica and multi-node placement, rolling-update bookkeeping, surge settlement/starts, and the StatefulSet assignment-plus-sticky-binding pairs -- with flush points placed before each store read-back so per-tick visibility is unchanged. The remaining reconcilers' proposals are per-instance failure-path events, deliberately left unbatched.
- **Confidence**: High
